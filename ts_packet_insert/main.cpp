...
#define TO_BE16(from, to)                                                      \
  *to = (0xFF & (from >> 8));                                                  \
  *(to + 1) = (0xFF & from);

static uint8_t *create_out_packet(char *in_data, int in_len, uint16_t pid) {
  uint8_t *out_packet = (uint8_t *)malloc(TS_PACKET_SIZE);
...
static int insert_packets(int file, int out_file, uint8_t *out_packet,
...
static void print_usage() {
  print_msg("Usage: ts_packet_insert [switches] <infile>\n"
            "\n");
  ...
  print_msg(
      ...
      "                    between 0 and 1, representing how far through to "
      "put \n"
      "                    each TS packet.  E.g., -p 0.1:0.4:0.7:0.9 will "
      "insert\n"
      "                    4 packets at 10%%, 40%%, 70%% and 90%% through the "
      "file.\n"
      "  -pid <pid>        The inserted packets will have the PID specfied.\n"
      "                    If no PID is specified, then 0x68 will be used.\n"
      "  -s <string>       The inserted packets will contain <string> as "
      "their\n"
      "                    payload. This defaults to 'Inserted packet'.\n"
      "  -o <output file>  The new TS file will be written out with the given "
      "name\n"
...
int main(int argc, char **argv) {
  ...
  while (argno < argc) {
    if (argv[argno][0] == '-') {
      if (!strcmp("-p", argv[argno])) {
        ...
        positions = (double *)malloc(n_pos * sizeof(double));
        if (!positions) {
        ...
        while (1) {
          ...
          if (endptr == position_string || positions[pos_index] > 1 ||
              positions[pos_index] < 0) {
            fprint_err("\nNot a valid floating point number for position "
                       "(argument %d)\n",
                       argno);
            exit(1);
          }
        ...
        int err;
        CHECKARG("ts_packet_insert", argno);
        err = unsigned_value("ts_packet_insert", argv[argno], argv[argno + 1],
                             0, &pid);
        if (err)
          return 1;
        argno++;
...

  {
...

    {
      ...
      packet_numbers = (int *)malloc(n_pos * sizeof(int));
...

      {
        ...
        uint8_t *out_packet =
            create_out_packet(out_string, strlen(out_string) + 1, pid);
