 ...
 */
static int write_ES_unit_as_TS(TS_writer_p output, ES_unit_p unit,
                               uint32_t video_pid) {
...
static int transfer_data(ES_p es, TS_writer_p output, uint32_t pmt_pid,
                         uint32_t video_pid, byte stream_type, int max2,
...
static void print_usage() {
  ...
  print_msg(
      ...
      "  (H.264) and AVS. Also supports MPEG-1 input streams, insofar as "
      "MPEG-2\n"
      ...
      "  it conforms to H.262, H.264 or AVS. It is important to get this "
      "right,\n"
...
int main(int argc, char **argv) {
  ...
  err = transfer_data(es, output, pmt_pid, video_pid, stream_type, max2,
                      verbose, quiet);
  if (err)
