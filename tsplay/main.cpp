...
static void print_usage(int summary) {
  ...
  if (summary)
    ...
    print_msg(
        ...
        "\n"
        "  Note that most switches can be placed anywhere on the command "
        "line.\n");
  print_msg(
      "\n"
      "Input:\n"
  ...
  if (summary)
    print_msg("  -stdout           Output is to standard output. Forces -quiet "
              "and -err stderr.\n");
  else
    print_msg("  -err stdout       Write error messages to standard output "
              "(the default)\n"
              "  -err stderr       Write error messages to standard error "
              "(Unix traditional)\n"
              "  -stdout           Output is to standard output. This does not "
              "make sense\n"
              "                    with -tcp or -udp. This forces -quiet and "
              "-err stderr.\n");
  print_msg(
      "\n"
      "  -mcastif <ipaddr>\n"
      ...
      "                    network interface to use. This may not be "
      "supported\n"
...
static void print_help_help() {
  print_msg(
      ...
      "Otherwise:\n"
      "\n"
      "  -help detail[s]   Show an expanded version of the help you get if "
      "you\n"
      "                    run tsplay with no arguments\n"
      "  -help ts          Show help specific to playing TS data\n"
      "  -help ps          Show help specific to playing PS data\n"
      "  -help tuning      Show help about how to tune the program's "
      "operation\n"
      "  -help test[ing]   Show help on switches that can be useful in "
      "testing\n"
...
static void print_help_ts() {
  print_msg(
      ...
      "                    (more accurately, any TS packets with this PID and "
      "with\n"
      "                    PCR information will be transmitted with PID "
      "0x1FFF, and\n"
      "                    any other packets with this PID will be ignored).\n"
      "                    The <pid> may not be 0, and should not be the PAT "
      "or PMT.\n"
      "\n"
      "Normally, the TS reading process remembers the last PCR, and also scans "
      "ahead\n"
      "for the next PCR. This allows it to calculate an accurate timestamp for "
      "each TS\n"
      "packet, at the cost of ignoring any packets before the first PCR, and "
      "also\n"
      "(at least in the currrent implementation), ignoring any TS packets "
      "after the\n"
      "last PCR, and also at the cost of some extra copying of data.\n"
      "\n"
      "However, sometimes it is more appropriate to revert to the mechanism "
      "used by\n"
      "earlier versions of tsplay:\n"
      "\n"
      "  -oldpace          Switch off scanning ahead for the next PCR.\n"
      "  -pace-pcr1        v1 of the PCR scan code\n"
      "  -pace-pcr2-ts     v2 of the PCR scan code - use 1st PCR PID found "
      "[default]\n"
      "  -pace-pcr2-pmt    v2 of the PCR scan code - get PCR PID from PMT\n"
      "\n"
      "which attempts to predict an approximate PCR for each TS packet, based "
      "on an\n"
      "initial speed (see '-bitrate'/'-byterate' in '-help tuning') and the "
      "PCRs found\n"
      "earlier in the data stream. This works reasonably well for streams with "
      "a\n"
      "constant bitrate, but does not cope well if the bitrate varies "
      "greatly.\n"
      "\n"
      "Note that '-nopcrs' (see '-help tuning') also implies '-oldpace'.\n"
      "\n"
      "In order to buffer PCRs, the first PCR must be found. Normally this is "
      "done\n"
      "by finding the first PAT/PMT, and reading the PCR PID from there. "
      "However,\n"
      "sometimes it is useful to *tell* the program where to look for its "
      "first PCR:\n"
      "\n"
      "  -forcepcr <pid>   Specifies which PID to look for to find the first "
      "PCR.\n"
      "\n"
      "Note that after the first PCR is read, *all* TS packets are inspected "
      "for\n"
...
static void print_help_ps() {
  print_msg(
      ...
      "\n"
      "Finally, it is occasionally useful to tweak how often PAT/PMT are "
      "written,\n"
      ...
      "                    TS packets, to allow a TS reader time to byte "
      "align\n"
...
int main(int argc, char **argv) {
  ...
  while (ii < argc) {
    ...
    if (argv[ii][0] == '-') {
      if (!strcmp("--help", argv[ii]) || !strcmp("-h", argv[ii]) ||
        ...
        if (argc == (ii + 1))
          ...
          if (!strcmp(argv[ii + 1], "ps"))
            ...
            print_help_tuning();
          else if (!strcmp(argv[ii + 1], "test") ||
                   !strcmp(argv[ii + 1], "testing"))
            print_help_testing();
          else if (!strcmp(argv[ii + 1], "debug") ||
                   !strcmp(argv[ii + 1], "debugging"))
            print_help_debugging();
          else if (!strcmp(argv[ii + 1], "detail") ||
                   !strcmp(argv[ii + 1], "details"))
            print_usage(false);
        ...
        CHECKARG("tsplay", ii);
        err =
            int_value((char *)"tsplay", argv[ii], argv[ii + 1], true, 10, &max);
        if (err)
          return 1;
        ii++;
        ...
        CHECKARG("tsplay", ii);
        err = unsigned_value((char *)"tsplay", argv[ii], argv[ii + 1], 0,
                             &override_pcr_pid);
        if (err)
          return 1;
        ii++;
        ...
        CHECKARG("tsplay", ii);
        err = int_value_in_range((char *)"ps2ts", argv[ii], argv[ii + 1], 0,
                                 0xF, 0, &video_stream);
        if (err)
          return 1;
        ii++;
        ...
        CHECKARG("tsplay", ii);
        err = int_value((char *)"tsplay", argv[ii], argv[ii + 1], true, 10,
        ...
        err = int_value((char *)"tsplay", argv[ii], argv[ii + 1], true, 0,
                        &drop_packets);
        if (err)
          return 1;
        err = int_value((char *)"tsplay", argv[ii], argv[ii + 2], true, 0,
                        &drop_number);
        if (err)
          return 1;
        ii += 2;
      ...
      if (!had_input_name) {
        ...
        err = host_value((char *)"tsplay", nullptr, argv[ii], &output_name,
                         &port);
        if (err)
          return 1;
        had_output_name = true;
  ...
  if (max > 0 && how == TS_W_UDP && (max / 7) < context.circ_buf_size) {
    fprint_err("### tsplay: -max %d cannot work with -buffer %d"
               ...
               max, context.circ_buf_size, context.circ_buf_size * 7);
    if (max / 7 > 0)
  ...
  if (!quiet) {
    if (is_TS) {
      ...
      if (pace_mode != TSPLAY_OUTPUT_PACE_FIXED)
        print_msg("Using 'exact' TS packet timing (by looking-ahead to the "
                  "next PCR)\n");
      else
  ...
  if (drop_packets) {
    if (!quiet)
      fprint_msg("DROPPING: Keeping %d TS packet%s, then dropping (throwing "
                 "away) %d\n",
  ...
  if (is_TS) {
    ...
    err = play_PS_stream(input, tswriter, pad_start, repeat_program_every,
                         force_stream_type, want_h262, input_is_dvd,
                         video_stream, audio_stream, want_ac3_audio,
                         want_dolby_as_dvb, pmt_pid, pcr_pid, video_pid, true,
                         audio_pid, max, loop, verbose, quiet);
  if (err) {
