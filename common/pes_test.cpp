...
int write_program_data_A(PES_reader_p reader, TS_writer_p output) {
  ...
  err = write_TS_program_data2(output,
                               ...
                               reader->program_number, pmt_pid, pcr_pid,
...
int play_pes_packets(PES_reader_p reader, TS_writer_p output) {
  ...
  for (;;) {
    ...
    err =
        write_ES_as_TS_PES_packet(output, item->unit.data, item->unit.data_len,
...
int test1(PES_reader_p reader, int verbose) {
  ...
  for (ii = 0; ii < 10; ii++) {
    ...
    if (verbose) {
      printf("\n>> PS packet at " OFFSET_T_FORMAT " is %02x (", packet->posn,
             packet->data[3]);
