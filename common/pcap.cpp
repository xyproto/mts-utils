...
inline uint32_t uint_32_ctx(const struct _pcap_io_ctx *const ctx,
                            const void *v) {
  return ctx->is_be ? uint_32_be(static_cast<const uint8_t *>(v))
                    : uint_32_le(static_cast<const uint8_t *>(v));
}

inline uint16_t uint_16_ctx(const struct _pcap_io_ctx *const ctx,
                            const void *v) {
...
inline uint64_t uint_64_be_ctx(const struct _pcap_io_ctx *const ctx,
                               const void *v) {
  return ((uint64_t)uint_32_ctx(ctx, v) << 32) |
         (uint64_t)uint_32_ctx(ctx, (const char *)v + 4);
}

inline uint64_t uint_64_ctx(const struct _pcap_io_ctx *const ctx,
                            const void *v) {
  return ctx->is_be
             ? ((uint64_t)uint_32_be(static_cast<const uint8_t *>(v)) << 32) |
                   (uint64_t)uint_32_be(static_cast<const uint8_t *>(v) + 4)
             : ((uint64_t)uint_32_le(static_cast<const uint8_t *>(v) + 4)
                << 32) |
                   (uint64_t)uint_32_le(static_cast<const uint8_t *>(v));
}

static int read_block_header(const struct _pcap_io_ctx *const ctx,
...
static int do_section_header(struct _pcap_io_ctx *const ctx, uint32_t length,
...
static int read_block(struct _pcap_io_ctx *const ctx,
                      pcapng_header_t *const hdr) {
  int rv = 1;
  int hdr_type;
  uint32_t length;
  ...
  case PCAPNG_TYPE_INTERFACE_BLOCK: {
    ...
    if (ctx->if_count + 1 > ctx->if_size) {
      if (ctx->interfaces == nullptr) {
        if ((ctx->interfaces = (pcapng_hdr_interface_t *)malloc(
                 sizeof(*ctx->interfaces) * 4)) == nullptr)
          return PCAP_ERR_OUT_OF_MEMORY;
        ctx->if_size = 4;
      } else {
...
static int pcap_read_header(PCAP_reader_p ctx, pcap_hdr_t *hdr) {
  ...
  // This reads an old-style header which is shorter than the shortest new-style
  // one
  ...
  if (magic == PCAPNG_TYPE_SECTION_HEADER_BLOCK) {
    ...
    if ((rv = do_section_header(ctx, uint_32_ctx(ctx, hdr_val + 4), hdr_val + 8,
                                &nghdr)) <= 0)
      return rv;
    ...
    hdr->version_major =
        (ctx->is_be ? uint_16_be(&hdr_val[4]) : uint_16_le(&hdr_val[4]));
    hdr->version_minor =
        (ctx->is_be ? uint_16_be(&hdr_val[6]) : uint_16_le(&hdr_val[6]));
    hdr->thiszone = (int32_t)(ctx->is_be ? uint_32_be(&hdr_val[8])
                                         : uint_32_le(&hdr_val[8]));
    hdr->sigfigs =
        (ctx->is_be ? uint_32_be(&hdr_val[12]) : uint_32_le(&hdr_val[12]));
    hdr->snaplen =
        (ctx->is_be ? uint_32_be(&hdr_val[16]) : uint_32_le(&hdr_val[16]));
    hdr->network =
        (ctx->is_be ? uint_32_be(&hdr_val[20]) : uint_32_le(&hdr_val[20]));
  }
...
static int pcap_read_pktheader(PCAP_reader_p ctx, pcaprec_hdr_t *hdr) {
  ...
  hdr->ts_sec =
      (ctx->is_be ? uint_32_be(&hdr_val[0]) : uint_32_le(&hdr_val[0]));
  hdr->ts_usec =
      (ctx->is_be ? uint_32_be(&hdr_val[4]) : uint_32_le(&hdr_val[4]));
  hdr->incl_len =
      (ctx->is_be ? uint_32_be(&hdr_val[8]) : uint_32_le(&hdr_val[8]));
  hdr->orig_len =
      (ctx->is_be ? uint_32_be(&hdr_val[12]) : uint_32_le(&hdr_val[12]));
