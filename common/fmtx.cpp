...
const TCHAR *fmtx_timestamp(int64_t n, unsigned int flags) {
  ...
  case FMTX_TS_DISPLAY_27MHz_RAW:
    _sntprintf(buf, FMTX_BUFFER_SIZE, _T("%") I64FMT _T("d:%03dt"), n27 / 300LL,
               frac_27MHz(n27));
    break;
  ...
  case FMTX_TS_DISPLAY_90kHz_32BIT: {
    ...
    if (n90 < 0)
      *p++ = _T('-');
    _sntprintf(p, FMTX_BUFFER_SIZE, _T("%ut"),
               (unsigned int)(n90 < 0 ? -n90 : n90));
    break;
  }
  ...
  case FMTX_TS_DISPLAY_HMS: {
    ...
    _sntprintf(buf, FMTX_BUFFER_SIZE, _T("%s%u:%02u:%02u.%04u"),
               n27 < 0 ? _T("-") : _T(""), h, m, s, f / 1000);
    break;
  }
  }
...
static const struct s2tsfss {
  ...
  int flags;
    ...
    {"hms", FMTX_TS_DISPLAY_HMS},      {"ms", FMTX_TS_DISPLAY_ms},
