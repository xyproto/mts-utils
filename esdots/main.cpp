...
double frame_rate =
    25.0; // default frame rate. this can be modified using the switch "-fr"
 ...
 */
static int h262_item_dot(h262_item_p item, double *delta_gop,
                         int show_gop_time) {
  char *str = nullptr;
  ...
  // print the time every time we find a random access point (time between two
  // GOPs)
  if (item->unit.start_code == 0xB3) {
    *delta_gop = (frames - temp_frames) /
                 frame_rate; // time between two GOPs [in seconds]
    temp_frames = frames;
 ...
 */
static int report_h262_file_as_dots(ES_p es, int max, int verbose,
                                    int show_gop_time) {
  int err;
  int count = 0;
  double time_gop = 0.0;
  ...
  fprint_msg(
      "GOP times (s): max=%2.4f, min=%2.4f, mean=%2.6f (frame rate = %2.2f)\n",
      time_gop_max, time_gop_min, time_gop_tot / (gops - 1), frame_rate);
  return 0;
}
  ...
  for (;;) {
    ...
    if (avs_frame->is_frame) {
      ...
      case 0xB7:
        ...
        break;
      default: /*print_msg("?");*/
        fprint_msg("<%x>", avs_frame->start_code);
        break;
      }
    }
...
static char choose_nal_type(access_unit_p access_unit, int *gop_start_found) {
  ...
  // true:  a random access point is identified as an I frame + recovery_point
  // SEI.
 ...
 */
static int dots_by_access_unit(ES_p es, int max, int verbose, int hash_eos,
                               int show_gop_time) {
  ...
  if (verbose)
    print_msg(
        ...
        "    _       means that the access unit doesn't contain a primary "
        "picture.\n"
        "\n"
    ...
    if (gop_start_found) {
      if (!is_first_k_frame) {
        ...
        if (show_gop_time)
          fprint_msg(": %2.4f\n",
                     (double)size_gop /
                         frame_rate); // that's the time duration of a "GOP"
  ...
  fprint_msg("\nFound %d NAL unit%s in %d access unit%s\n", context->nac->count,
             (context->nac->count == 1 ? "" : "s"), access_unit_count,
  ...
  if (gops) // only if there is more than 1 gop
    fprint_msg(
        "GOP size (s): max=%2.4f, min=%2.4f, mean=%2.5f (frame rate = %2.2f)\n",
 ...
 * Returns 0 if it succeeds, 1 if some error occurs.
 */
static int report_file_as_ES_dots(ES_p es, int what_data, int max,
                                  int verbose) {
  int err = 0;
  int count = 0;
  struct ES_unit unit;
  ...
  for (;;) {
    ...
    case VIDEO_AVS:
      ...
      case 0xB6:
        ...
        case AVS_P_PICTURE_CODING:
          print_msg("p");
          break;
        case AVS_B_PICTURE_CODING:
          print_msg("b");
          break;
        default:
          print_msg("!");
...
static void print_usage() {
  print_msg("Usage: esdots [switches] [<infile>]\n"
            "\n");
  ...
  print_msg(
      ...
      "  -hasheos          Print a # on finding an EOS (end-of-stream) NAL "
      "unit\n"
      ...
      "                    OR the distance between random access points "
      "(H.264)\n"
      ...
      "\n"
      "  For AVS data, the program will never guess correctly, so the user "
      "must\n"
...
int main(int argc, char **argv) {
  ...
  err = open_input_as_ES((use_stdin ? nullptr : input_name), use_pes, false,
