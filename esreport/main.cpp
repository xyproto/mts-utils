...
static void report_avs_frames(ES_p es, int max, int verbose, int quiet, int count_sizes)
{
    ...
    fprint_msg("Found %d AVS 'frame'%s:\n"
               ...
               "   %5d sequence end%s\n",
        count, (count == 1 ? "" : "s"), num_frames, (num_frames == 1 ? "" : "s"),
        num_x_frames[AVS_I_PICTURE_CODING], num_x_frames[AVS_P_PICTURE_CODING],
...

    {
        ...
        fprint_msg(
            "At 25 frames/second, that is %dm %.1fs (%.2fs)\n", minutes, seconds, total_seconds);
    }
    ...
    if (count_sizes) {
        ...
        if (num_frames > 0)
            fprint_msg("Frame sizes ranged from %5u to %7u bytes, mean %9.2f\n", min_frame_size,
                max_frame_size, sum_frame_size / (double)num_frames);
        for (ii = 0; ii < 3; ii++) {
            if (num_x_frames[ii] > 0)
                fprint_msg("          %s frames from %5u to %7u bytes, mean %9.2f\n",
                    (ii == 0 ? "I" : ii == 1 ? "P" : ii == 2 ? "B" : "?"), min_x_frame_size[ii],
                    max_x_frame_size[ii], sum_x_frame_size[ii] / (double)num_x_frames[ii]);
        }
...
static void find_h262_fields(ES_p es, int max, int verbose)
{
    ...
    fprint_msg("Found %d MPEG-2 'picture'%s:\n"
               ...
               "   %5d sequence header%s\n"
               "   %5d sequence end%s\n",
        count, (count == 1 ? "" : "s"), num_fields, (num_fields == 1 ? "" : "s"), num_frames,
        (num_frames == 1 ? "" : "s"), num_sequence_headers, (num_sequence_headers == 1 ? "" : "s"),
...
static void report_h262_frames(ES_p es, int max, int verbose, int quiet, int count_sizes)
{
    ...
    fprint_msg("Found %d MPEG-2 'picture'%s:\n"
               ...
               "   %5d sequence header%s\n"
               "   %5d sequence end%s\n",
        count, (count == 1 ? "" : "s"), num_frames, (num_frames == 1 ? "" : "s"), num_x_frames[0],
        num_x_frames[1], num_x_frames[2], num_x_frames[3], num_sequence_headers,
        (num_sequence_headers == 1 ? "" : "s"), num_sequence_ends,
        (num_sequence_ends == 1 ? "" : "s"));
    ...
    if (count_sizes) {
        ...
        for (ii = 0; ii < 4; ii++) {
            if (num_x_frames[ii] > 0)
                fprint_msg("          %s frames from %5u to %7u bytes, mean %9.2f\n",
                    H262_PICTURE_CODING_STR(ii), min_x_frame_size[ii], max_x_frame_size[ii],
...
static void report_by_nal_unit(ES_p es, int max, int quiet, int show_nal_details)
{
    ...
    if (err == 0 || err == EOF) {
        ...
        for (ii = 0; ii < 10; ii++)
            if (slice_types[ii] > 0)
                fprint_msg(
                    "  %8d of type %2d (%s)\n", slice_types[ii], ii, NAL_SLICE_TYPE_STR(ii));
    } else
...
static void find_h264_fields(ES_p es, int max, int quiet, int verbose, int show_nal_details)
{
    ...
    fprint_msg("Found %d MPEG-4 picture%s, %d field%s, %d frame%s\n", count,
        (count == 1 ? "" : "s"), num_fields, (num_fields == 1 ? "" : "s"), num_frames,
        (num_frames == 1 ? "" : "s"));
...
#define I_SLICE_MIX 3
    ...
    if (count_sizes && access_unit_count > 0)
        fprint_msg("Frame sizes ranged from %u to %u bytes, mean %.2f\n", min_frame_size,
            max_frame_size, sum_frame_size / (double)access_unit_count);
...
int main(int argc, char** argv)
{
    ...
    if (report_ES) {
        ...
        if (find_fields)
            ...
            report_h264_frames(
                es, max, quiet, verbose, show_nal_details, report_framesize, report_frametype);
        else
