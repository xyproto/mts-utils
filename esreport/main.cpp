 ...
 */
static void report_avs_frames(ES_p es, int max, int verbose, int quiet,
                              int count_sizes) {
  int err;
  int count = 0;
  int num_frames = 0;
  ...
  for (;;) {
    ...
    if (frame->is_frame) {
      if (count_sizes) {
        ...
        if (err)
          break;
        if (min_frame_size > length)
          min_frame_size = length;
        ...
        if (frame->picture_coding_type < 3) // paranoia - check in array bounds
        {
          ...
          num_x_frames[ii]++;
          if (min_x_frame_size[ii] > length)
            min_x_frame_size[ii] = length;
          if (max_x_frame_size[ii] < length)
            max_x_frame_size[ii] = length;
      ...
      if (count_sizes) {
        ...
        if (err)
          break;
        if (min_seq_hdr_size > length)
          min_seq_hdr_size = length;
        if (max_seq_hdr_size < length)
          max_seq_hdr_size = length;
  ...
  fprint_msg("Found %d AVS 'frame'%s:\n"
             ...
             "   %5d sequence end%s\n",
             count, (count == 1 ? "" : "s"), num_frames,
             (num_frames == 1 ? "" : "s"), num_x_frames[AVS_I_PICTURE_CODING],
             num_x_frames[AVS_P_PICTURE_CODING],
             num_x_frames[AVS_B_PICTURE_CODING], num_sequence_headers,
             (num_sequence_headers == 1 ? "" : "s"), num_sequence_ends,
...
static void find_h262_fields(ES_p es, int max, int verbose) {
  ...
  fprint_msg("Found %d MPEG-2 'picture'%s:\n"
             ...
             "   %5d sequence header%s\n"
             "   %5d sequence end%s\n",
             count, (count == 1 ? "" : "s"), num_fields,
             (num_fields == 1 ? "" : "s"), num_frames,
             (num_frames == 1 ? "" : "s"), num_sequence_headers,
             (num_sequence_headers == 1 ? "" : "s"), num_sequence_ends,
             (num_sequence_ends == 1 ? "" : "s"));
}
 ...
 * - if `count_sizes` is true, then a summary of frame sizes will be kept
 */
static void report_h262_frames(ES_p es, int max, int verbose, int quiet,
                               int count_sizes) {
  int err;
  int count = 0;
  int num_frames = 0;
      ...
      if (count_sizes) {
        ...
        if (picture->picture_coding_type < 5 &&
            picture->picture_coding_type >
                0) // paranoia - check for array bounds
        {
          // I, P, B or D frame
  ...
  fprint_msg("Found %d MPEG-2 'picture'%s:\n"
             ...
             "   %5d sequence end%s\n",
             count, (count == 1 ? "" : "s"), num_frames,
             (num_frames == 1 ? "" : "s"), num_x_frames[0], num_x_frames[1],
             num_x_frames[2], num_x_frames[3], num_sequence_headers,
             (num_sequence_headers == 1 ? "" : "s"), num_sequence_ends,
             (num_sequence_ends == 1 ? "" : "s"));
  ...
  if (count_sizes) {
    ...
    for (ii = 0; ii < 4; ii++) {
      if (num_x_frames[ii] > 0)
        fprint_msg("          %s frames from %5u to %7u bytes, mean %9.2f\n",
                   H262_PICTURE_CODING_STR(ii), min_x_frame_size[ii],
                   max_x_frame_size[ii],
                   sum_x_frame_size[ii] / (double)num_x_frames[ii]);
    }
    if (num_sequence_headers > 0) {
...
static void report_h262_afds(ES_p es, int max, int verbose, int quiet) {
  ...
  for (;;) {
    ...
    if (picture->is_picture && picture->afd != afd) {
      ...
      int minutes = (int)(total_seconds / 60);
      double seconds = total_seconds - 60 * minutes;
      fprint_msg("%dm %4.1fs (frame %d @ %.2fs): ", minutes, seconds, frames,
                 total_seconds);
      report_h262_picture(picture, false);
      afd = picture->afd;
    }
...
static void report_h262_items(ES_p es, int max, int verbose, int quiet) {
  ...
  for (;;) {
    ...
    if (verbose)
      print_data(true, "        Data", item->unit.data, item->unit.data_len,
                 10);
    free_h262_item(&item);
 ...
 */
static void report_by_nal_unit(ES_p es, int max, int quiet,
 ...
 * - if `quiet` is true, then only errors will be reported
 */
static void find_h264_fields(ES_p es, int max, int quiet, int verbose,
                             int show_nal_details) {
  int err;
  int count = 0;
  int num_fields = 0;
  ...
  fprint_msg("Found %d MPEG-4 picture%s, %d field%s, %d frame%s\n", count,
             (count == 1 ? "" : "s"), num_fields, (num_fields == 1 ? "" : "s"),
 ...
 */
static void report_h264_frames(ES_p es, int max, int quiet, int verbose,
                               int show_nal_details, int count_sizes,
                               int count_types) {
  int err = 0;
...
#define I_SLICE_MIX 3
  ...
  fprint_msg("Found %d frame%s (%d NAL unit%s)\n", access_unit_count,
             (access_unit_count == 1 ? "" : "s"), context->nac->count,
...
static void print_usage() {
  print_msg("Usage: esreport [switches] [<infile>]\n"
            "\n");
  ...
  print_msg(
      ...
      "  -afd              Report (just) on AFD changes in H.262. Ignored for "
      "the\n"
      "                    other types of file.\n"
      ...
      "                    in each MPEG-2 item. For ES units, output "
      "information\n"
      "                    about the data in each ES unit. Ignored for H.264 "
      "data.\n"
      ...
      "  -max <n>, -m <n>  Maximum number of NAL units/MPEG-2 items/AVS "
      "frames/ES units\n"
      "                    to read. If -frames, then the program will stop "
      "after\n"
      "                    that many frames. If reading 'frames', MPEG-2 and "
      "AVS will\n"
      ...
      "  the file to determine if the stream is H.264, H.262 or AVS data. "
      "This\n"
