 ...
 */
static int extract_av_via_pes(char *input_name, char *output_name,
                              int want_video, int quiet) {
  int err;
 ...
 * Returns 0 if all went well, 1 if something went wrong.
 */
static int extract_pid_packets(TS_reader_p tsreader, FILE *output,
                               uint32_t pid_wanted, int max, int verbose,
                               int quiet) {
  int err;
  int count = 0;
  int extracted = 0;
 ...
 * Returns 0 if all went well, 1 if something went wrong.
 */
static int extract_av(int input, FILE *output, int want_video, int max,
  ...
  for (;;) {
    ...
    for (ii = 0; ii < pmt->num_streams; ii++) {
      if ((want_video && IS_VIDEO_STREAM_TYPE(pmt->streams[ii].stream_type)) ||
          (!want_video &&
           (IS_AUDIO_STREAM_TYPE(pmt->streams[ii].stream_type)))) {
  ...
  if (pid == 0) {
    fprint_err(
        "### No %s stream specified in first %d TS packets in input file\n",
  ...
  if (!quiet)
    fprint_msg("Extracting %s PID %04x (%d)\n",
               (want_video ? "video" : "audio"), pid, pid);
 ...
 * Returns 0 if all went well, 1 if something went wrong.
 */
static int extract_pid(int input, FILE *output, uint32_t pid_wanted, int max,
                       int verbose, int quiet) {
  int err;
  TS_reader_p tsreader = nullptr;
...
static void print_usage() {
  print_msg("Usage: ts2es [switches] [<infile>] [<outfile>]\n"
            "\n");
...
int main(int argc, char **argv) {
  ...
  if (use_pes) {
    err = extract_av_via_pes(input_name, output_name,
                             (extract == EXTRACT_VIDEO), quiet);
    if (err) {
  ...
  if (extract == EXTRACT_PID)
    ...
    err = extract_av(input, output, (extract == EXTRACT_VIDEO), max, verbose,
                     quiet);
  if (err) {
