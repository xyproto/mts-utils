...
int main(int argc, char** argv)
{
    ...
    if (use_pes) {
        err = extract_av_via_pes(input_name, output_name, (extract == EXTRACT_VIDEO), quiet);
        if (err) {
    ...
    if (extract == EXTRACT_PID)
        ...
        err = extract_av(input, output, (extract == EXTRACT_VIDEO), max, verbose, quiet);
    if (err) {
    ...
    if (!use_stdout) {
        ...
        if (err) {
            fprint_err(
                "### ts2es: Error closing output file %s: %s\n", output_name, strerror(errno));
            (void)close_file(input);
            return 1;
        }
    }
