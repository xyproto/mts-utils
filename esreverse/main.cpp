 ...
 */
static int reverse_h262(ES_p es, WRITER output, int max, int frequency,
                        int as_TS, int verbose, int quiet) {
  int err = 0;
  ...
  if (as_TS)
    err = output_in_reverse_as_TS(es, output.ts_output, frequency, verbose,
                                  quiet, -1, 0, reverse_data);
  else
    err = output_in_reverse_as_ES(es, output.es_output, frequency, verbose,
                                  quiet, -1, 0, reverse_data);
 ...
 * Returns 0 if all went well, 1 if something went wrong.
 */
static int output_parameter_sets(WRITER output, access_unit_context_p context,
                                 int as_TS, int quiet) {
 ...
 * Returns 0 if all went well, 1 if something went wrong.
 */
static int reverse_access_units(ES_p es, WRITER output, int max, int frequency,
                                int as_TS, int verbose, int quiet) {
  int err = 0;
  reverse_data_p reverse_data = nullptr;
...
int main(int argc, char **argv) {
  ...
  while (ii < argc) {
    if (argv[ii][0] == '-') {
      if (!strcmp("--help", argv[ii]) || !strcmp("-help", argv[ii]) ||
        ...
        CHECKARG("esreverse", ii);
        err = host_value("esreverse", argv[ii], argv[ii + 1], &output_name,
                         &port);
        if (err)
          return 1;
        had_output_name = true; // more or less
        ...
        CHECKARG("esreverse", ii);
        err = int_value("esreverse", argv[ii], argv[ii + 1], true, 10,
                        &frequency);
        if (err)
          return 1;
        ii++;
  ...
  err = open_input_as_ES(input_name, use_pes, quiet, force_stream_type,
  ...
  if (is_data == VIDEO_H262)
    ...
    err =
        reverse_access_units(es, output, max, frequency, as_TS, verbose, quiet);
