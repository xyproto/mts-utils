 ...
 */
static int check_if_TS(int input, byte cur_byte, int verbose, int *decided,
                       int *result) {
  int ii;
  ...
  if (verbose)
    print_msg(
        "  First byte in file is 0x47, so it looks like Transport Stream\n");
...
static int check_if_PS(int input, int verbose, int *decided, int *result) {
  ...
  if (buf[0] != 0 || buf[1] != 0 || buf[2] != 1 || buf[3] != 0xba) {
    if (verbose)
      fprint_msg(
          "  File starts %02X %02X %02X %02X, not 00 00 01 BA - not PS\n",
  ...
  if ((buf[0] & 0xF0) == 0x20) {
    ...
    if (err) {
      fprint_err(
          "### %s trying to read last 2 bytes of body of PS pack header\n",
          (err == EOF ? "EOF" : "Error"));
      return 1;
    }
  ...
  if (buf[0] != 0 || buf[1] != 0 || buf[2] != 1) {
    if (verbose)
      fprint_msg(
          "  Next 'packet' starts %02X %02X %02X, not 00 00 01 - not PS\n",
          buf[0], buf[1], buf[2]);
    return 0;
  }
 ...
 */
static int determine_packet_type(int input, int verbose, int *decided,
                                 int *result) {
  int err;
  ssize_t length;
  byte first_byte;
...
static void print_usage() {
  print_msg("Usage: stream_type [switches] <infile>\n"
            "\n");
  ...
  print_msg(
      ...
      "  - for TS, the first byte in the file will be the start of a NAL "
      "unit,\n"
