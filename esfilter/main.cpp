...
static int copy_h262(ES_p es, WRITER output, int as_TS, int max, int quiet)
{
    ...
    for (;;) {
        ...
        if (as_TS)
            err = write_ES_as_TS_PES_packet(output.ts_output, item->unit.data, item->unit.data_len,
...
static int strip_h262(
    ES_p es, WRITER output, int as_TS, int max, int keep_p, int verbose, int quiet)
{
    int err;
    int count;
...
static int filter_h262(
    ES_p es, WRITER output, int as_TS, int frequency, int max, int verbose, int quiet)
{
    int err;
    int count = 0;
    h262_context_p h262 = NULL;
    ...
    if (!quiet) {
        ...
        fprint_msg("Frames      %10d %10d (%4.1f%%) %10d (%4.1f%%)\n", pictures_seen,
            pictures_kept, 100 * (((double)pictures_kept) / pictures_seen), pictures_written,
...
static int strip_access_units(
    ES_p es, WRITER output, int as_TS, int max, int keep_all_ref, int verbose, int quiet)
{
    int err = 0;
    int count;
    ...
    for (count = 1;; count++) {
        ...
        if (as_TS)
            err = write_access_unit_as_TS(
                access_unit, fcontext->access_unit_context, output.ts_output, DEFAULT_VIDEO_PID);
        else
            err = write_access_unit_as_ES(
                access_unit, fcontext->access_unit_context, output.es_output);
        if (err) {
            print_err("### Error writing picture\n");
            free_h264_filter_context(&fcontext);
            free_access_unit_context(&acontext);
...
static int filter_access_units(
    ES_p es, WRITER output, int as_TS, int max, int frequency, int verbose, int quiet)
{
    int err = 0;
    int count;
    access_unit_context_p acontext = NULL;
    ...
    if (!quiet) {
        ...
        print_msg("            Found       Kept            Written\n");
        fprint_msg("Frames %10d %10d (%4.1f%%) %10d (%4.1f%%)\n", access_units_seen,
            access_units_kept, 100 * (((double)access_units_kept) / access_units_seen),
            access_units_written, 100 * (((double)access_units_written) / access_units_seen));
        if (frequency != 0)
            fprint_msg("Target (frames) . %10d (%4.1f%%) at requested"
                       " frequency %d\n",
...
static int do_action(ACTION action, ES_p es, WRITER output, int max, int frequency, int is_h262,
    int as_TS, int keep_all_ref, byte stream_type, int verbose, int quiet)
{
    int err = 0;
...
int main(int argc, char** argv)
{
    ...
    err = do_action(action, es, output, max, frequency, want_data == VIDEO_H262, as_TS,
        keep_all_ref, stream_type, verbose, quiet);
    if (err) {
