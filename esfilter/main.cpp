 ...
 * Returns 0 if it succeeds, 1 if some error occurs.
 */
static int write_h262_picture(WRITER output, int as_TS,
                              h262_picture_p picture) {
  int err;
  if (as_TS)
    err =
        write_h262_picture_as_TS(output.ts_output, picture, DEFAULT_VIDEO_PID);
  else
 ...
 * Returns 0 if it succeeds, 1 if some error occurs.
 */
static int strip_h262(ES_p es, WRITER output, int as_TS, int max, int keep_p,
  ...
  for (count = 1;; count++) {
    ...
    err = get_next_stripped_h262_frame(fcontext, verbose, quiet, &seq_hdr,
                                       &picture, &delta_pictures_seen);
    if (err == EOF) {
      if (!quiet)
        print_msg("EOF\n");
      break;
  ...
  if (!quiet) {
    fprint_msg("Found %d frames, kept %d (%.1f%%)\n", pictures_seen,
               pictures_kept, 100.0 * pictures_kept / pictures_seen);
  }
  return 0;
}
 ...
 * Returns 0 if it succeeds, 1 if some error occurs.
 */
static int filter_h262(ES_p es, WRITER output, int as_TS, int frequency,
  ...
  if (!quiet) {
    ...
    fprint_msg(
        ...
        pictures_kept, 100 * (((double)pictures_kept) / pictures_seen),
        pictures_written, 100 * (((double)pictures_written) / pictures_seen));
    if (frequency != 0)
      fprint_msg("Target (frames)     .  %10d (%4.1f%%) at requested"
                 " frequency %d\n",
                 pictures_seen / frequency, 100.0 / frequency, frequency);
  }
  return 0;
}
 ...
 */
static int copy_nal_units(ES_p es, WRITER output, int as_TS, int max,
                          int verbose, int quiet) {
  int err = 0;
 ...
 * Returns 0 if all went well, 1 if something went wrong.
 */
static int strip_access_units(ES_p es, WRITER output, int as_TS, int max,
                              int keep_all_ref, int verbose, int quiet) {
  int err = 0;
  int count;
  ...
  for (count = 1;; count++) {
    ...
    err = get_next_stripped_h264_frame(fcontext, verbose, quiet, &access_unit,
 ...
 * Returns 0 if all went well, 1 if something went wrong.
 */
static int filter_access_units(ES_p es, WRITER output, int as_TS, int max,
                               int frequency, int verbose, int quiet) {
  int err = 0;
  int count;
  access_unit_context_p acontext = nullptr;
  h264_filter_context_p fcontext = nullptr;
  ...
  for (count = 1;; count++) {
    ...
    err = get_next_filtered_h264_frame(
        fcontext, verbose, quiet, &this_access_unit, &delta_access_units_seen);
    if (err == EOF)
      break;
    else if (err) {
      print_err("### Error getting next filtered picture\n");
    ...
    if (this_access_unit != nullptr) {
      if (as_TS)
        ...
        err = write_access_unit_as_ES(
            this_access_unit, fcontext->access_unit_context, output.es_output);
      if (err) {
        print_err("### Error writing picture\n");
  ...
  if (!quiet) {
    ...
    if (frequency != 0)
      fprint_msg("Target (frames) . %10d (%4.1f%%) at requested"
                 " frequency %d\n",
                 access_units_seen / frequency, 100.0 / frequency, frequency);
  }
  return 0;
}
 ...
 * Returns 0 if all went well, 1 if something went wrong.
 */
static int do_action(ACTION action, ES_p es, WRITER output, int max,
                     int frequency, int is_h262, int as_TS, int keep_all_ref,
                     byte stream_type, int verbose, int quiet) {
  int err = 0;
  ...
  if (as_TS) {
    ...
    err = write_TS_program_data(output.ts_output, 1, 1, DEFAULT_PMT_PID,
                                DEFAULT_VIDEO_PID, stream_type);
    if (err)
      return 1;
  }
  ...
  case ACTION_FILTER:
    if (is_h262)
      ...
      err = filter_access_units(es, output, as_TS, max, frequency, verbose,
                                quiet);
    break;
  ...
  case ACTION_STRIP:
    if (is_h262)
      ...
      err = strip_access_units(es, output, as_TS, max, keep_all_ref, verbose,
                               quiet);
    break;
...
static void print_usage() {
  print_msg("Usage: esfilter [actions/switches] [<infile>] [<outfile>]\n"
            "\n");
...
int main(int argc, char **argv) {
  ...
  while (ii < argc) {
    if (argv[ii][0] == '-') {
      if (!strcmp("--help", argv[ii]) || !strcmp("-help", argv[ii]) ||
        ...
        CHECKARG("esfilter", ii);
        err =
            host_value("esfilter", argv[ii], argv[ii + 1], &output_name, &port);
        if (err)
          return 1;
        had_output_name = true; // more or less
        use_tcpip = true;
        ...
        CHECKARG("esfilter", ii);
        err =
            int_value("esfilter", argv[ii], argv[ii + 1], true, 10, &frequency);
        if (err)
          return 1;
        ii++;
      } else {
  ...
  err = open_input_as_ES((use_stdin ? nullptr : input_name), use_pes, quiet,
  ...
  if (as_TS) {
    if (use_stdout)
      err = tswrite_open(TS_W_STDOUT, nullptr, nullptr, 0, quiet,
                         &(output.ts_output));
    else if (use_tcpip)
      err = tswrite_open(TS_W_TCP, output_name, nullptr, port, quiet,
                         &(output.ts_output));
    else
      err = tswrite_open(TS_W_FILE, output_name, nullptr, 0, quiet,
                         &(output.ts_output));
    if (err) {
