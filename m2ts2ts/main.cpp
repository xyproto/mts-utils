...
{
    packet_buffer->timestamp = (((uint32_t)(packet_buffer->m2ts_packet[0])) << 24)
...
static int extract_packets(
    int input, FILE* output, const unsigned int reorder_buffer_size, bool verbose, bool quiet)
{
    int err;
...
int main(int argc, char* argv[])
{
    ...
    if (use_stdout) {
        ...
        if (output == nullptr) {
            ...
            fprint_err(
                "### m2ts2ts: Unable to open output file %s: %s\n", output_name, strerror(errno));
            return 1;
        }
    }
    if (!quiet)
