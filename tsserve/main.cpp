...
#define EXTRACT_REVERSE_FROM_STREAM(stream)                                    \
  ((stream).is_h262 ? (stream).u.h262->reverse_data                            \
                    : (stream).u.h264->reverse_data)

/*
 * Note that `program_number` should be 1 or more.
 */
static int build_stream(ES_p es, int is_h262, int program_number,
...
static int build_filter_context(stream_context stream, int is_strip,
                                int frequency, filter_context *fcontext) {
  int err;
  fcontext->is_h262 = stream.is_h262;
  if (stream.is_h262) {
    if (is_strip)
      err = build_h262_filter_context_strip(&(fcontext->u.h262), stream.u.h262,
                                            true);
    else
      err = build_h262_filter_context(&(fcontext->u.h262), stream.u.h262,
                                      frequency);
  } else {
    if (is_strip)
      err = build_h264_filter_context_strip(&(fcontext->u.h264), stream.u.h264,
                                            true);
    else
      err = build_h264_filter_context(&(fcontext->u.h264), stream.u.h264,
                                      frequency);
  }
  return err;
}
 ...
 * and pictures.
 */
static inline int get_next_picture(stream_context stream, int verbose,
                                   int quiet, picture *pic) {
  int err;
  if (stream.is_h262) {
...
static inline int is_reference_picture(picture pic) {
  if (pic.is_h262)
    ...
    return (pic.u.h264->primary_start != nullptr &&
...
static inline int write_picture_as_TS(stream_context stream, TS_writer_p output,
                                      picture pic) {
  ...
  if (stream.is_h262)
    ...
    return write_access_unit_as_TS(pic.u.h264, stream.u.h264, output,
                                   es->reader->output_video_pid);
}
...
static inline int get_next_stripped(filter_context fcontext, int verbose,
                                    int quiet, picture *seq_hdr,
  ...
  if (fcontext.is_h262) {
    ...
    err =
        get_next_stripped_h262_frame(fcontext.u.h262, verbose, quiet, &_seq_hdr,
...
static inline int get_next_filtered(filter_context fcontext, int verbose,
                                    int quiet, picture *seq_hdr,
                                    picture *this_picture,
                                    int *delta_pictures_seen) {
  int err;
  ...
  if (fcontext.is_h262) {
    ...
    h262_picture_p _seq_hdr = nullptr;
    err =
        get_next_filtered_h262_frame(fcontext.u.h262, verbose, quiet, &_seq_hdr,
                                     &_this_picture, delta_pictures_seen);
    seq_hdr->u.h262 = _seq_hdr;
    this_picture->u.h262 = _this_picture;
  } else {
 ...
 */
static int play_normal(stream_context stream, TS_writer_p output, int verbose,
                       int quiet, int num_normal, int tsdirect,
                       reverse_data_p reverse_data) {
  int err;
  ES_p es = EXTRACT_ES_FROM_STREAM(stream);
  ...
  if (extra_info)
    print_msg("Playing at normal speed\n");
  ...
  if (stream.is_h262) {
    ...
    err =
        collect_reverse_access_units(stream.u.h264, num_normal, verbose, quiet);
    if (err)
      return err;
  }
  return 0;
 ...
 * if the current command has changed.
 */
static int flush_after_normal(stream_context stream, TS_writer_p output,
  ...
  if (item_start.infile < reader->packet->posn) {
    ...
    picture picture;
    if (extra_info)
      print_msg(".. which is in the previous packet - "
    ...
    if (extra_info)
      fprint_msg(".. new last item starts at " OFFSET_T_FORMAT "/%d\n",
                 item_start.infile, item_start.inpacket);
  }
  ...
  if (item_start.inpacket == 0) {
    ...
    if (extra_info)
      print_msg(".. so there's no need to output any of this"
                " packet\n");
    return 0;
  }
  ...
  if (extra_info)
    fprint_msg(".. so need to output %d bytes of this"
               " packet\n",
               item_start.inpacket);

  err = write_ES_as_TS_PES_packet(output, reader->packet->es_data,
                                  item_start.inpacket, reader->output_video_pid,
...
static int output_next_reference_picture(stream_context stream,
                                         TS_writer_p output, int verbose,
                                         int quiet, int I_only) {
  int err;
  picture picture;

  if (extra_info)
    print_msg(".. outputting next reference picture\n");
 ...
 * if the current command has changed.
 */
static int resync_after_reverse(stream_context stream, TS_writer_p output,
                                int verbose, int quiet) {
  int err;
  ES_p es = EXTRACT_ES_FROM_STREAM(stream);
  ...
  if (extra_info)
    fprint_msg(
        "   triple byte = %02x,%02x,%02x, next byte to be from " OFFSET_T_FORMAT
        "/%d\n",
 ...
 * if the current command has changed.
 */
static int resync_after_filter(stream_context stream, TS_writer_p output,
                               int verbose, int quiet) {
  int err;
 ...
 * if the current command has changed.
 */
static int back_to_normal(stream_context stream, TS_writer_p output,
                          int tsdirect) {
  int err;
  ES_p es = EXTRACT_ES_FROM_STREAM(stream);
  PES_reader_p reader = es->reader;
  ...
  if (item_start.infile < reader->packet->posn) {
    ...
    if (stream.is_h262) {
      ...
      if (extra_info)
        fprint_msg(".. next byte is %d, so length wanted is %d"
                   " - outputting it\n",
                   curposn, length_wanted);
      err = write_ES_as_TS_PES_packet(
          output, stream.u.h262->last_item->unit.data, length_wanted,
          reader->output_video_pid, DEFAULT_VIDEO_STREAM_ID);
    } else {
      ...
      if (extra_info)
        fprint_msg(".. next byte is %d, so length wanted is %d"
                   ...
                   curposn, length_wanted);
      err = write_ES_as_TS_PES_packet(
          output, stream.u.h264->pending_nal->unit.data, length_wanted,
          reader->output_video_pid, DEFAULT_VIDEO_STREAM_ID);
    }
    if (err) {
    ...
    if (extra_info)
      print_msg(".. i.e., at start of packet, nothing to do\n");
  } else {
    ...
    err = write_ES_as_TS_PES_packet(
        output, &reader->packet->es_data[start_offset], length_wanted,
        reader->output_video_pid, DEFAULT_VIDEO_STREAM_ID);
    if (err) {
 ...
 * if the current command has changed.
 */
static int play_stripped(stream_context stream, filter_context fcontext,
                         TS_writer_p output, int verbose, int quiet,
                         int tsdirect, int num_fast, int with_seq_hdrs) {
  int err;
  ES_p es = EXTRACT_ES_FROM_STREAM(stream);
  PES_reader_p reader = es->reader;
  ...
  if (extra_info)
    print_msg("Fast forwarding (strip)\n");
  ...
  for (;;) {
    ...
    err = get_next_stripped(fcontext, verbose, quiet, &seq_hdr, &this_picture,
                            &delta_pictures_seen);
 ...
 * if the current command has changed.
 */
static int play_filtered(stream_context stream, filter_context fcontext,
                         TS_writer_p output, int verbose, int quiet,
                         int tsdirect, int num_faster, int frequency,
                         int with_seq_hdrs) {
  int err;
  ES_p es = EXTRACT_ES_FROM_STREAM(stream);
  PES_reader_p reader = es->reader;
  ...
  if (extra_info)
    print_msg("Fast forwarding (filter)\n");
  ...
  for (;;) {
    ...
    err = get_next_filtered(fcontext, verbose, quiet, &seq_hdr, &this_picture,
                            &delta_pictures_seen);
    if (err == EOF || err == COMMAND_RETURN_CODE) {
      free_picture(&last_picture);
      break;
 ...
 * if the current command has changed.
 */
static int skip_forwards(stream_context stream, TS_writer_p output,
                         filter_context fcontext, int with_seq_hdrs,
                         int num_to_skip, int verbose, int quiet,
                         int tsdirect) {
  int err;
  ES_p es = EXTRACT_ES_FROM_STREAM(stream);
  PES_reader_p reader = es->reader;
  ...
  if (extra_info)
    fprint_msg("Skipping forwards (%d frames)\n", num_to_skip);
 ...
 * if the current command has changed.
 */
static int skip_backwards(stream_context stream, TS_writer_p output,
                          int num_to_skip, int verbose, int quiet, int tsdirect,
                          reverse_data_p reverse_data) {
  int err;
  ES_p es = EXTRACT_ES_FROM_STREAM(stream);
  PES_reader_p reader = es->reader;
  ...
  err = output_in_reverse_as_TS(es, output, num_to_skip, verbose, quiet, -1,
                                num_to_skip, reverse_data);
  if (err && err != COMMAND_RETURN_CODE) {
 ...
 * if the current command has changed.
 */
static int play_reverse(stream_context stream, TS_writer_p output, int verbose,
                        int quiet, int tsdirect, int frequency, int num_reverse,
                        reverse_data_p reverse_data) {
  int err;
  ES_p es = EXTRACT_ES_FROM_STREAM(stream);
  PES_reader_p reader = es->reader;
  ...
  err = output_in_reverse_as_TS(es, output, frequency, verbose, quiet, -1,
 ...
 */
static int obey_command(char this_command, char last_command, int *index,
                        ...
                        TS_writer_p tswriter, int video_only, int verbose,
                        int quiet, int tsdirect, int with_seq_hdrs,
  ...
  for (;;) {
    ...
    case COMMAND_NORMAL:
      ...
      err = play_normal(stream[which], tswriter, verbose, quiet, 0, tsdirect,
                        reverse_data[which]);
    ...
    case COMMAND_FAST:
      ...
      err = play_stripped(stream[which], scontext[which], tswriter, verbose,
                          quiet, tsdirect, 0, with_seq_hdrs);
      break;
    ...
    case COMMAND_FAST_FAST:
      ...
      err = play_filtered(stream[which], fcontext[which], tswriter, verbose,
                          quiet, tsdirect, 0, ffrequency, with_seq_hdrs);
      break;
    ...
    case COMMAND_REVERSE:
      ...
      stop_server_output(reader[which]);
      set_PES_reader_video_only(reader[which], true);
      err = play_reverse(stream[which], tswriter, verbose, quiet, tsdirect,
                         rfrequency, 0, reverse_data[which]);
      if (err == 0) {
        if (!quiet)
          fprint_msg("Start of file %d\n", which);
    ...
    case COMMAND_SKIP_FORWARD:
      ...
      set_PES_reader_video_only(reader[which], true);
      err =
          skip_forwards(stream[which], tswriter, fcontext[which], with_seq_hdrs,
    ...
    case COMMAND_SKIP_BACKWARD_LOTS:
      ...
      stop_server_output(reader[which]);
      set_PES_reader_video_only(reader[which], true);
      err = skip_backwards(stream[which], tswriter, BIG_SKIP_DISTANCE, verbose,
                           quiet, tsdirect, reverse_data[which]);
      this_command = COMMAND_NORMAL; // aim to continue with normal play
      break;
...
#if 0 // The following would only make sense if we *knew* we'd just been doing
      // 'n'ormal play... Try to ensure we finish at the end of a picture...
        err = flush_after_normal(stream[which],tswriter,verbose,quiet);
...
static int play(int default_index, PES_reader_p reader[MAX_INPUT_FILES],
                ...
                filter_context scontext[MAX_INPUT_FILES],
                reverse_data_p reverse_data[MAX_INPUT_FILES],
                TS_writer_p tswriter, int video_only, int verbose, int quiet,
                int tsdirect, int with_seq_hdrs, int ffrequency,
                int rfrequency) {
  int err;
  int ii;
  int started[MAX_INPUT_FILES];
...
#if 0 // Shouldn't need to do this, as any command that *does* anything will
      // output it
  ...
  for (;;) {
    ...
    err = obey_command(this_command, last_command, &which, started, reader,
                       stream, fcontext, scontext, reverse_data, tswriter,
...
static int play_pes_packets(PES_reader_p reader[MAX_INPUT_FILES],
                            TS_writer_p tswriter, tsserve_context_p context,
                            int verbose, int quiet) {
  int err;
  int ii;
  ...
  for (ii = 0; ii < MAX_INPUT_FILES; ii++) {
    ...
    err = build_filter_context(stream[ii], false, context->ffrequency,
                               &fcontext[ii]);
    if (err) {
  ...
  err = play(context->default_file_index, reader, stream, fcontext, scontext,
             reverse_data, tswriter, context->video_only, verbose, quiet,
             context->tsdirect, context->with_seq_hdrs, context->ffrequency,
             context->rfrequency);
 ...
 * Returns 0 if all went well, 1 if an error occurred.
 */
static int test_play(PES_reader_p reader, stream_context stream,
                     filter_context fcontext, filter_context scontext,
                     reverse_data_p reverse_data, TS_writer_p tswriter,
                     int video_only, int verbose, int quiet, int tsdirect,
                     int num_normal, int num_fast, int num_faster,
                     int num_reverse, int ffrequency, int rfrequency,
                     int with_seq_hdrs) {
  int err = 0;
  int started = false;
  int ii;
  ...
  if (num_fast == 0 && num_faster == 0 && num_reverse == 0) {
    ...
    err = play_normal(stream, tswriter, verbose, quiet, tsdirect, 0,
                      reverse_data);
    if (err == EOF)
      return 0;
    else
  ...
  for (ii = 0; ii < 2; ii++) {
    ...
    set_PES_reader_video_only(reader, video_only);
    err = play_normal(stream, tswriter, verbose, quiet, tsdirect, num_normal,
                      reverse_data);
    if (err == EOF)
      break;
    else if (err)
    ...
    err = play_filtered(stream, fcontext, tswriter, verbose, quiet, tsdirect,
                        num_faster, ffrequency, with_seq_hdrs);
    if (err == EOF)
      break;
    else if (err)
      return 1;
    ...
    set_PES_reader_video_only(reader, true);
    err = play_reverse(stream, tswriter, verbose, quiet, rfrequency, tsdirect,
                       num_reverse, reverse_data);
    if (err == EOF)
      break;
    else if (err)
 ...
 * Returns 0 if all went well, 1 if an error occurred.
 */
static int test_skip(PES_reader_p reader, stream_context stream,
                     filter_context fcontext, filter_context scontext,
                     reverse_data_p reverse_data, TS_writer_p tswriter,
                     int video_only, int verbose, int quiet, int tsdirect,
                     int with_seq_hdrs) {
  ...
  for (ii = 0; ii < 1; ii++) {
    ...
    stop_server_output(reader);
    set_PES_reader_video_only(reader, true);
    err = skip_backwards(stream, tswriter, 1, verbose, quiet, tsdirect,
                         reverse_data);
    if (err == EOF)
      break;
    else if (err)
...
static int test_play_pes_packets(PES_reader_p reader, TS_writer_p tswriter,
                                 tsserve_context_p context, int pad_start,
                                 int video_only, int verbose, int quiet,
                                 int tsdirect, int num_normal, int num_fast,
                                 int num_faster, int num_reverse,
                                 int ffrequency, int rfrequency, int skiptest,
                                 int with_seq_hdrs) {
  int err;
  int ii;
  ...
  if (reader->is_h264) {
    ...
    if (skiptest)
      err = test_skip(reader, stream, fcontext, scontext, reverse_data,
                      tswriter, video_only, verbose, quiet, tsdirect, false);
    else
      err =
          test_play(reader, stream, fcontext, scontext, reverse_data, tswriter,
                    video_only, verbose, quiet, tsdirect, num_normal, num_fast,
                    num_faster, num_reverse, ffrequency, rfrequency, false);
    ...
    if (skiptest)
      err =
          test_skip(reader, stream, fcontext, scontext, reverse_data, tswriter,
                    video_only, verbose, quiet, tsdirect, with_seq_hdrs);
    else
      err = test_play(reader, stream, fcontext, scontext, reverse_data,
                      tswriter, video_only, verbose, quiet, tsdirect,
                      num_normal, num_fast, num_faster, num_reverse, ffrequency,
                      rfrequency, with_seq_hdrs);
...
static int open_input_file(tsserve_context_p context, int quiet, int verbose,
                           PES_reader_p *reader) {
  ...
  if (!(*reader)->is_TS) {
    set_PES_reader_program_data(*reader, 1, context->pmt_pid,
                                context->video_pid, context->audio_pid,
                                context->pcr_pid);
...
static int open_input_files(tsserve_context_p context, int quiet, int verbose,
                            PES_reader_p reader[MAX_INPUT_FILES]) {
  int ii;
  for (ii = 0; ii < MAX_INPUT_FILES; ii++) {
    int err;
    ...
    err =
        open_PES_reader(context->input_names[ii], !quiet, verbose, &reader[ii]);
    if (err) {
    ...
    if (!quiet)
      fprint_msg("Opened input file %2d, %s, as %s\n", ii,
                 context->input_names[ii], (reader[ii]->is_TS ? "TS" : "PS"));
    ...
    if (!reader[ii]->is_TS && context->force_stream_type &&
      ...
      if (!quiet)
        fprint_msg("File appeared to contain %s, forcing %s\n",
                   reader[ii]->is_h264 ? "MPEG-4/AVC (H.264)"
                                       : "MPEG-2 (H.272)",
                   context->want_h262 ? "MPEG-2" : "MPEG-4/AVC");
...
static int tsserve_child_process(struct server_args *args) {
  ...
  if (!quiet)
    fprint_msg("Establishing connection with client on socket %d\n",
  ...
  // And play...
  if (!quiet)
    fprint_msg("Playing to client via socket %d\n", tswriter->where.socket);
  ...
  if (!quiet)
    print_msg("Finished talking to client\n");
 ...
 */
static int start_child(tsserve_context_p context, TS_writer_p tswriter,
                       int verbose, int quiet) {
  pid_t pid;
...
static void set_child_exit_handler() {
  ...
  action.sa_flags =
      SA_NOCLDSTOP; // we only want terminated children, not stopped children
#ifdef SA_RESTART
  ...
  if (ret < 0)
    print_err(
        "!!! tsserve: Error starting signal handler to reap child processes\n");
}

/*
 * Run as a server
 */
static int run_server(tsserve_context_p context, int listen_port, int verbose,
                      int quiet) {
  int err;
 ...
 * Run tests
 */
static int test_reader(tsserve_context_p context, int output_to_file,
                       char *output_name, int port, int num_normal,
                       int num_fast, int num_faster, int num_reverse,
                       int skiptest, int verbose, int quiet, int tsdirect) {
  int err;
  TS_writer_p tswriter = nullptr;
  PES_reader_p reader = nullptr;

  err = tswrite_open((output_to_file ? TS_W_FILE : TS_W_TCP), output_name,
                     nullptr, port, quiet, &tswriter);
  if (err) {
  ...
  // And play...
  err = test_play_pes_packets(reader, tswriter, context, context->pad_start,
                              context->video_only, verbose, quiet, tsdirect,
 ...
 */
static int command_reader(tsserve_context_p context, char *output_name,
                          int port, int use_stdin, int verbose, int quiet) {
  int err;
  int ii, had_err;
  TS_writer_p tswriter = nullptr;
  PES_reader_p reader[MAX_INPUT_FILES];
...
static void print_usage() {
  ...
  print_msg(
      ...
      "  -err stdout       Write error messages to standard output (the "
      "default)\n"
      "  -err stderr       Write error messages to standard error (Unix "
      "traditional)\n"
      ...
      "\n"
      "  -noseqhdr         Do not output sequence headers for fast "
      "forward/reverse\n"
      ...
      "\n"
      "  -h264, -avc       Force the program to treat the input as "
      "MPEG-4/AVC.\n"
      "  -h262             Force the program to treat the input as MPEG-2.\n"
      "  Both of these affect the stream type of the output data.\n"
      "\n"
      "  If the audio stream being output is Dolby (AC-3), then the stream "
      "type\n"
...
static void print_detailed_usage() {
  print_msg(
      ...
      "                    If given before any of -0..-9, this will be "
      "treated\n"
      "                    as a specification of file 0. If given after "
      "-0..-9,\n"
      ...
      "                    instead of just sending the PES packets for the "
      "video\n"
      "                    and audio streams'\n"
      "\n"
      "  Note that when -tsdirect is specified, PES packets are still "
      "inspected\n"
      "  to allow building up the fast forward/reverse indices.\n"
      "  Also, -prepeat, -pes_padding and -drop will have no effect with this "
      "switch.\n"
      "\n"
      "Other stuff:\n"
      "\n"
      ...
      "\n"
      "  -pes_padding <n>  When outputting in 'normal play' mode, input PES "
      "packets\n"
      "                    are copied to the output. If '-pes_padding' is "
      "used, then <n>\n"
      "                    dummy PES packets will be added to the output for "
      "each input\n"
      "                    packet, causing the amount of data output to be "
      "roughly <n>+1\n"
      "                    times as great. This can be useful for benchmarking "
      "the recipient.\n"
      "\n"
      "  -drop <k> <d>     As TS packets are output, for every <k>+<d> "
      "packets,\n"
      "                    keep <k> and then drop (throw away) <d>.\n"
      "                    Applies to all TS packets output, regardless of "
      "selected file.\n"
      "                    This can be useful when testing other "
      "applications.\n"
      "\n"
      "Alternate modes\n"
      "---------------\n"
...
int main(int argc, char **argv) {
  ...
  while (argno < argc) {
    if (argv[argno][0] == '-') {
      if (!strcmp("--help", argv[argno]) || !strcmp("-h", argv[argno]) ||
        ...
        context.tsdirect =
            true; // Write to server as a side effect of TS reading
        ...
        CHECKARG("tsserve", argno);
        err = host_value("tsserve", argv[argno], argv[argno + 1], &output_name,
                         &output_port);
        if (err)
          return 1;
        ...
        argno++;
      } else if (!strcmp("-output", argv[argno]) ||
                 !strcmp("-o", argv[argno])) {
        CHECKARG("tsserve", argno);
        output_to_file = true;
  ...
  if (context.drop_packets && !quiet)
    fprint_msg(
        "DROPPING: Keeping %d TS packet%s, then dropping (throwing away) %d\n",
  ...
  case ACTION_CMD:
    err = command_reader(&context, output_name, output_port, use_stdin, verbose,
                         quiet);
    if (err) {
      fprint_err("### tsserve: Error playing to %s\n", output_name);
      return 1;
