...
enum class Extract { Undefined = 0, TS = 1, PID = 2 };
...
static const uint8_t *page_composition_segment(dvbdata_t *const dvbd,
                                               const uint8_t *p,
                                               int segment_length) {
  const uint8_t *const eos = p + segment_length;
  const char *state_text[] = {"normal", "acquisition point", "mode change",
                              "reserved"};
  int page_state;
...
static const uint8_t *region_composition_segment(dvbdata_t *const dvbd,
                                                 const uint8_t *p,
                                                 int segment_length) {
  const uint8_t *const eos = p + segment_length;
...
static const uint8_t *CLUT_definition_segment(dvbdata_t *const dvbd,
                                              const uint8_t *p,
                                              int segment_length) {
  const uint8_t *const eos = p + segment_length;
...
static const uint8_t *object_data_segment(dvbdata_t *const dvbd,
                                          const uint8_t *p,
                                          int segment_length) {
  ...
  case 0: {
    ...
    p += 2;
    print_data(true, "top pixel-data:", p, top_field_data_block_length,
               0x10000);
    p += top_field_data_block_length;
    print_data(true, "bottom pixel-data:", p, bottom_field_data_block_length,
               0x10000);
...
static const uint8_t *subtitling_segment(dvbdata_t *const dvbd,
                                         const uint8_t *p) {
...
static void flush_dvbd(dvbdata_t *const dvbd) {
  ...
  fprint_msg("\nPTS: %s, DTS: %s, PTS - last_PTS: %s\n",
             ...
             !dvbd->pts_valid
                 ? "????"
                 : fmtx_timestamp(dvbd->pts - dvbd->last_pts, tfmt));
  if (dvbd->pts_valid)
  ...
  if (dvbd->data_len > (unsigned int)(p - dvbd->data)) {
    print_data(true, "excess bytes", p, dvbd->data_len - (p - dvbd->data),
               0x10000);
...
static void add_data_dvbd(dvbdata_t *const dvbd, const uint8_t *const data,
                          unsigned int len) {
 ...
 */
static int extract_pid_packets(TS_reader_p tsreader, uint32_t pid_wanted,
  ...
  for (;;) {
    ...
    if (pid == pid_wanted) {
      ...
      if (payload_unit_start_indicator) {
        ...
        err = find_PTS_DTS_in_PES(payload, payload_len, &dvbd.pts_valid,
                                  &dvbd.pts, &dvbd.dts_valid, &dvbd.dts);
        dvbd.found = true;
 ...
 * Returns 0 if all went well, 1 if something went wrong.
 */
static int extract_av(int input, const int prog_no, int max, int verbose,
                      int quiet) {
  int err, ii;
  int max_to_read = max;
  ...
  for (;;) {
    ...
    err = find_pmt(tsreader, prog_no, max_to_read, verbose, quiet, &num_read,
                   &pmt);
    if (err == EOF) {
      if (!quiet)
  ...
  if (pid == 0) {
    fprint_err("### No DVB subtitle stream specified in first %d TS packets in "
               "input file\n",
               max);
    free_TS_reader(&tsreader);
    return 1;
 ...
 * Returns 0 if all went well, 1 if something went wrong.
 */
static int extract_pid(int input, uint32_t pid_wanted, int max, int verbose,
                       int quiet) {
  int err;
  TS_reader_p tsreader = nullptr;
...
static void print_usage() {
  ...
  print_msg(
      ...
      "\n"
      "Files:\n"
      "  <infile>  is an H.222 Transport Stream file (but see -stdin and "
      "-pes)\n"
      "\n"
