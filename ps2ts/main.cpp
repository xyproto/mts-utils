...
static void print_usage() {
  print_msg("Usage: ps2ts [switches] [<infile>] [<outfile>]\n"
            "\n");
  ...
  print_msg(
      ...
      "  it conforms to H.262, H.264, etc. It is important to get this right, "
      "as\n"
      ...
      "  -mp42             Force the program to treat the input as MPEG-4/Part "
      "2.\n"
      "  -vtype <type>     Force the program to treat the input as video of\n"
      "                    stream type <type> (e.g., 0x42 means AVS video). It "
      "is\n"
...
int main(int argc, char **argv) {
  ...
  if (!quiet) {
    ...
    if (keep_audio) {
      ...
      if (audio_stream == -1)
        fprint_msg("first %s found",
                   (want_ac3_audio ? "AC3 stream" : "stream"));
      else
