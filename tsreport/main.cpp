...
static uint64_t estimate_pcr(offset_t posn, uint64_t ppcr_pos, uint64_t ppcr_val, double pcr_rate)
{
...
static int report_buffering_stats(TS_reader_p tsreader, const int req_prog_no, int max,
    bool verbose, bool quiet, char* output_name, uint32_t continuity_cnt_pid, uint64_t report_mask)
{
    pmt_p pmt = nullptr;
    int err;
    FILE* file = nullptr;
...
#define MAX_NUM_STREAMS 100
    ...
    for (;;) {
        ...
        err = split_TS_packet(packet, &pid, &payload_unit_start_indicator, &adapt, &adapt_len,
            &payload, &payload_len);
        if (err) {
        ...
        if (pid == pcr_pid) {
            ...
            if (got_pcr) {
                ...
                if (predict.know_pcr_rate) {
                    ...
                    uint64_t guess_pcr = estimate_pcr(
                        posn, predict.prev_pcr_posn, predict.prev_pcr, predict.pcr_rate);
                ...
                if (predict.had_a_pcr) {
                    if (pcr_signed_diff(predict.prev_pcr, adapt_pcr) > 0) {
                        fprint_err("!!! PCR %s at TS packet " OFFSET_T_FORMAT
                    ...
                    if (payload) {
                        if (((ss->last_cc + 1) & 15) != cc) {
                            ...
                            if (ss->err_cc_error++ == 0) {
                                fprint_msg("### PID(%d): Continuity Counter discontinuity %d->%d "
                                           "at " OFFSET_T_FORMAT "\n",
                        ...
                        if (continuity_cnt_pid == pid)
                            fprintf(file_cnt, "[Discontinuity] ");
                        if (ss->err_cc_error++ == 0) {
                            fprint_msg("### PID(%d): Continuity Counter discontinuity %d->%d (but "
        ...
        if (index != -1 && payload && payload_unit_start_indicator) {
            ...
            err = find_PTS_DTS_in_PES(
                payload, payload_len, &got_pts, &stats[index].pts, &got_dts, &stats[index].dts);
            if (err) {
                fprint_err(
                    "### PID(%d): Error looking for PTS/DTS in TS packet at " OFFSET_T_FORMAT "\n",
        ...
        fprint_msg("  First stream %d DTS (after first PCR) at " OFFSET_T_FORMAT "\n",
            ...
            if (file) {
                ...
                fprintf(file, OFFSET_T_FORMAT ",%s," LLU_FORMAT ",%d,%s,", posn,
                    ...
                    IS_AUDIO_STREAM_TYPE(stats[index].stream_type)
                        ? "audio"
                ...
                if (got_escr) {
                    if (!quiet)
                        fprint_msg(
                            "Found ESCR " LLU_FORMAT " at " OFFSET_T_FORMAT "\n", escr, posn);
            ...
            if (verbose) {
                fprint_msg(OFFSET_T_FORMAT_8 ": %s PCR " LLU_FORMAT " %d %5s", posn,
                    (pcr_pid == pid && got_pcr) ? "    " : "calc", pcr_time_now_div300, index,
                    IS_AUDIO_STREAM_TYPE(stats[index].stream_type)
                        ? "audio"
                        : IS_VIDEO_STREAM_TYPE(stats[index].stream_type) ? "video" : "");
            }
    ...
    for (ii = 0; ii < num_streams; ii++) {
        ...
        if (ss->pcr_pts_diff.num > 0) {
            fprint_msg("  PCR/%s:\n    Minimum difference was %6s at DTS %8s, TS packet "
                ...
                fmtx_timestamp(ss->pcr_pts_diff.min_at, tfmt_abs), ss->pcr_pts_diff.min_posn);
            fprint_msg("    Maximum difference was %6s at DTS %8s, TS packet at " OFFSET_T_FORMAT_8
                ...
                fmtx_timestamp(ss->pcr_pts_diff.max_at, tfmt_abs), ss->pcr_pts_diff.max_posn);
            ...
            fprint_msg("    Mean difference (of %u) is %s\n", ss->pcr_pts_diff.num,
                fmtx_timestamp(
                    (int64_t)(ss->pcr_pts_diff.sum / (double)ss->pcr_pts_diff.num), tfmt_diff));
        }
        ...
        if (ss->pcr_dts_diff.num > 0 && ss->pts_ne_dts) {
            fprint_msg("  PCR/DTS:\n    Minimum difference was %6s at DTS %8s, TS packet "
                ...
                fmtx_timestamp(ss->pcr_dts_diff.min_at, tfmt_abs), ss->pcr_dts_diff.min_posn);
            fprint_msg("    Maximum difference was %6s at DTS %8s, TS packet at " OFFSET_T_FORMAT_8
                       "\n",
                fmtx_timestamp(ss->pcr_dts_diff.max, tfmt_diff),
                fmtx_timestamp(ss->pcr_dts_diff.max_at, tfmt_abs), ss->pcr_dts_diff.max_posn);
            fprint_msg("    i.e., a span of %s\n",
                fmtx_timestamp(ss->pcr_dts_diff.max - ss->pcr_dts_diff.min, tfmt_diff));
            fprint_msg("    Mean difference (of %u) is %s\n", ss->pcr_dts_diff.num,
                fmtx_timestamp(
                    (int64_t)(ss->pcr_dts_diff.sum / (double)ss->pcr_dts_diff.num), tfmt_diff));
        }
        if (ss->had_a_dts) {
 ...
 * Returns 0 if all went well, 1 if something went wrong.
 */
static int report_ts(TS_reader_p tsreader, int max, bool verbose, int show_data, int report_timing)
{
 ...
 * Returns 0 if all went well, 1 if something went wrong.
 */
static int report_single_pid(TS_reader_p tsreader, int max, bool quiet, uint32_t just_pid)
{
    int err;
    int count = 0;
    int pid_count = 0;
...
int main(int argc, char** argv)
{
    ...
    if (select_pid)
        ...
        err = report_buffering_stats(tsreader, req_prog_no, max, verbose, quiet, output_name,
            continuity_cnt_pid, report_mask);
    else
