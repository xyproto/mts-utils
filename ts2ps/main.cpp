...
static int write_pack_header(FILE *output) {
  static byte pack_header[] = {0x00, 0x00, 0x01, 0xBA, 0x44, 0x00, 0x04,
 ...
 */
static int write_PES_packet(FILE *output, byte *data, uint16_t data_len,
                            byte stream_id) {
  static byte header[] = {0x00, 0x00, 0x01, 0xFF, 0xFF, 0xFF, // replace 3 bytes
 ...
 * Returns 0 if all went well, 1 if something went wrong.
 */
static int extract_data(int input, FILE *output, uint16_t program_number,
  ...
  for (;;) {
    ...
    // the data...
    if (reader->packet->data[4] == 0 && reader->packet->data[5] == 0) {
...
#define MAX_LENGTH 0xFFFF
      if (PES_packet_length > MAX_LENGTH) {
        fprint_err("PES packet of 'zero' length is really %6d - too long for "
                   "one packet\n",
                   PES_packet_length);
        ...
        count =
            fwrite(reader->packet->data, reader->packet->data_len, 1, output);
        if (count != 1) {
...
static void print_usage() {
  print_msg("Usage: ts2ps [switches] [<infile>] [<outfile>]\n"
            "\n");
