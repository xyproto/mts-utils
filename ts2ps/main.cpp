 ...
 */
static int write_PES_packet(FILE* output, byte* data, uint16_t data_len, byte stream_id)
{
...
static int extract_data(
    int input, FILE* output, uint16_t program_number, int max, bool verbose, bool quiet)
{
    int err;
    PES_reader_p reader;
...
#define MAX_LENGTH 0xFFFF
            if (PES_packet_length > MAX_LENGTH) {
                ...
                while (PES_packet_length > 0) {
                    ...
                    int this_length = std::min(MAX_LENGTH - 3, PES_packet_length);
                    int err;
                    fprint_err(".. writing out %5d\n", this_length);
                    err = write_PES_packet(output, start, this_length, reader->packet->data[3]);
                    if (err) {
                ...
                fprint_err("PES packet of 'zero' length, adjusting to %6d-6=%6d"
                    ...
                    reader->packet->data_len, PES_packet_length, reader->packet->data[3],
...
int main(int argc, char** argv)
{
    ...
    if (!use_stdout) {
        ...
        if (err) {
            fprint_err(
                "### ts2ps: Error closing output file %s: %s\n", output_name, strerror(errno));
            (void)close_file(input);
            return 1;
        }
    }
