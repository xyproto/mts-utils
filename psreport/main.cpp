...
#define cDEPTH 32
  ...
  for (;;) {
    ...
    if (verbose)
      fprint_msg("\n" OFFSET_T_FORMAT_08 ": Pack header: SCR " LLD_FORMAT
                 " (" LLD_FORMAT "/%d) mux rate %d\n",
                 posn, header.scr, header.scr_base, header.scr_extn,
    ...
    for (;;) {
      ...
      if (verbose) {
        fprint_msg(OFFSET_T_FORMAT_08 ": PS Packet %2d stream %02X (", posn,
                   count, stream_id);
...
#else
          report_PES_data_array("          ", packet.data, packet.data_len,
                                true);
#endif
      }
      ...
      if (stream_id == 0xBC)
        ...
        if (what != SUBSTREAM_ERROR) {
          ...
          case SUBSTREAM_OTHER:
            ...
            break;
          default:
            fprint_err("Internal error: got substream id %d"
    ...
    if (num_private[cPRIVATE1] > 0) {
      ...
      for (ii = 0; ii < cDEPTH; ii++) {
        if (num_other[cLPCM][ii] > 0) {
          fprint_msg("     LPCM, index %2d:        %8d", ii,
                     num_other[cLPCM][ii]);
          fprint_msg("  min size %5d, max size %5d, mean size %7.1f\n",
      ...
      for (ii = 0; ii < cDEPTH; ii++) {
        if (num_other[cSUBPICTURES][ii] > 0) {
          fprint_msg("     SUBPICTURES, index %2d: %8d", ii,
                     num_other[cSUBPICTURES][ii]);
          fprint_msg("  min size %5d, max size %5d, mean size %7.1f\n",
                     min_other_size[cSUBPICTURES][ii],
                     max_other_size[cSUBPICTURES][ii],
                     sum_other_size[cSUBPICTURES][ii] /
                         num_other[cSUBPICTURES][ii]);
        }
      }
      for (ii = 0; ii < cDEPTH; ii++) {
        if (num_other[cOTHER][ii] > 0) {
          fprint_msg("     OTHER, index %2d:       %8d", ii,
                     num_other[cOTHER][ii]);
          fprint_msg("  min size %5d, max size %5d, mean size %7.1f\n",
...
static void print_usage() {
  print_msg("Usage: psreport [switches] [<infile>]\n"
            "\n");
  ...
  print_msg(
      ...
      "                     The DVD specification stores AC-3 (Dolby), DTS "
      "and\n"
      "                     other audio in a specialised manner in "
      "private_stream_1.\n");
}
