...
#define cDEPTH 32
    ...
    for (;;) {
        ...
        if (verbose)
            fprint_msg("\n" OFFSET_T_FORMAT_08 ": Pack header: SCR " LLD_FORMAT " (" LLD_FORMAT
                       "/%d) mux rate %d\n",
                posn, header.scr, header.scr_base, header.scr_extn, header.program_mux_rate);
        ...
        for (;;) {
            ...
            if (stream_id == 0xbb) // System header
            {
                ...
                if (verbose)
                    fprint_msg(
                        OFFSET_T_FORMAT_08 ": System header %d\n", posn, num_system_headers);
            } else
                break;
        }
        ...
        for (;;) {
            ...
            if (stream_id == 0xBC)
                ...
                int what = identify_private1_data(
                    &packet, is_dvd, verbose, &substream_index, &bsmod, &acmod);
        ...
        for (ii = 0; ii < NUMBER_VIDEO_STREAMS; ii++)
            if (num_video[ii] > 0) {
                ...
                fprint_msg("  min size %5d, max size %5d, mean size %7.1f\n", min_video_size[ii],
                    max_video_size[ii], sum_video_size[ii] / num_video[ii]);
            }
        for (ii = 0; ii < NUMBER_AUDIO_STREAMS; ii++)
            if (num_audio[ii] > 0) {
                fprint_msg("Audio packets (stream %2d):  %8d", ii, num_audio[ii]);
                fprint_msg("  min size %5d, max size %5d, mean size %7.1f\n", min_audio_size[ii],
                    max_audio_size[ii], sum_audio_size[ii] / num_audio[ii]);
            }
        if (num_private[cPRIVATE1] > 0) {
            ...
            for (ii = 0; ii < cDEPTH; ii++) {
                if (num_other[cSUBPICTURES][ii] > 0) {
                    fprint_msg(
                        "     SUBPICTURES, index %2d: %8d", ii, num_other[cSUBPICTURES][ii]);
                    fprint_msg("  min size %5d, max size %5d, mean size %7.1f\n",
...
int main(int argc, char** argv)
{
    ...
    if (err) {
        fprint_err(
            "### psreport: Error closing input file %s\n", (use_stdin ? "<stdin>" : input_name));
        return 1;
    }
    return 0;
}
