 ...
 */
static int read_TS_packet(TS_reader_p tsreader, uint32_t *count,
                          byte *data[TS_PACKET_SIZE], uint32_t *pid,
                          int *got_pcr, uint64_t *pcr, int max, int loop,
 ...
 */
static int find_PCR_PID(TS_reader_p tsreader, TS_writer_p tswriter,
                        uint32_t *pcr_pid, uint32_t *num_read, int max,
                        int quiet) {
  int err;
  int count = 0;
  byte *data;
  ...
  for (;;) {
    ...
    if (pid == 0x0000 && !got_PAT) {
      ...
      err = build_psi_data(false, payload, payload_len, pid, &pat_data,
      ...
      err =
          extract_prog_list_from_pat(false, pat_data, pat_data_len, &prog_list);
      if (err != 0) {
        free(pat_data);
        continue;
      }
      ...
      if (!quiet)
        fprint_msg("Packet %d %s PMT with PID %04x\n", count,
                   payload_unit_start_indicator ? "starts" : "continues",
                   pmt_pid);
      ...
      err = build_psi_data(false, payload, payload_len, pid, &pmt_data,
...
static int play_buffered_TS_packets(TS_reader_p tsreader, TS_writer_p tswriter,
                                    ...
                                    uint32_t override_pcr_pid, int max,
  ...
  count = start_count;
  for (;;) {
    err = read_buffered_TS_packet(tsreader, &count, &data, &pid, &pcr, max,
                                  loop, start_posn, start_count, quiet);
...
static int play_TS_packets(TS_reader_p tsreader, TS_writer_p tswriter,
                           ...
                           uint32_t pid_to_ignore, int max, int loop, int quiet,
                           int verbose) {
  int err;
  int total = 0;
  uint32_t count = 0;
  ...
  for (;;) {
    ...
    err = read_TS_packet(tsreader, &count, &data, &pid, &got_pcr, &pcr, max,
                         loop, start_posn, start_count, quiet);
...
int play_TS_stream(int input, TS_writer_p tswriter,
  ...
  if (pace_mode == TSPLAY_OUTPUT_PACE_PCR1)
    ...
    err = play_TS_packets(tsreader, tswriter, pace_mode, pid_to_ignore, max,
                          loop, quiet, verbose);
  if (err) {
...
int play_PS_stream(int input, TS_writer_p output, int pad_start,
  ...
  if (force_stream_type) {
    ...
    if (!quiet)
      fprint_msg("Reading input as %s\n",
                 (want_h262 ? "MPEG-2 (H.262)" : "MPEG-4/AVC (H.264)"));
  } else {
  ...
  err = ps_to_ts(ps, output, pad_start, program_repeat, is_h264, input_is_dvd,
                 video_stream, audio_stream, want_ac3_audio, want_dolby_as_dvb,
                 pmt_pid, pcr_pid, video_pid, keep_audio, audio_pid, max,
                 verbose, quiet);
  ...
  if (loop) {
    for (;;) {
      if (!quiet)
        print_msg("Rewinding and continuing\n");
      ...
      err = ps_to_ts(ps, output, pad_start, program_repeat, is_h264,
                     input_is_dvd, video_stream, audio_stream, want_ac3_audio,
                     want_dolby_as_dvb, pmt_pid, pcr_pid, video_pid, keep_audio,
                     audio_pid, max, false, true);
      if (err) {
        if (loop)
          print_err("!!! Ignoring error and looping\n");
        else {
