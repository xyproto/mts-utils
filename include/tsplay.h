        ...
        if (pid == 0x0000 && !got_PAT) {
            ...
            if (!quiet)
                fprint_msg("Packet %d %s PMT with PID %04x\n", count,
                    payload_unit_start_indicator ? "starts" : "continues", pmt_pid);
    ...
    uint32_t pid_to_ignore, uint32_t override_pcr_pid, int max, int loop, int quiet, bool verbose)
{
    ...
    count = start_count;
    for (;;) {
        err = read_buffered_TS_packet(
            tsreader, &count, &data, &pid, &pcr, max, loop, start_posn, start_count, quiet);
    ...
    bool verbose)
{
    ...
    if (!quiet) {
        fprint_msg("Transferred %d TS packet%s in total\n", total, (total == 1 ? "" : "s"));
        fprint_msg(
            "Used PCRs from %d packets, ignored PCRs from %d packets\n", pcrs_used, pcrs_ignored);
    }
    return 0;
}
    ...
    uint32_t pid_to_ignore, uint32_t override_pcr_pid, int max, int loop, int quiet, bool verbose)
{
    ...
    if (pace_mode == TSPLAY_OUTPUT_PACE_PCR1)
        err = play_buffered_TS_packets(
            tsreader, tswriter, pid_to_ignore, override_pcr_pid, max, loop, quiet, verbose);
    else
        err = play_TS_packets(
            tsreader, tswriter, pace_mode, pid_to_ignore, max, loop, quiet, verbose);
    if (err) {
    ...
    int quiet)
{
    ...
    if (force_stream_type) {
        ...
        if (!quiet)
            fprint_msg(
                "Video appears to be %s\n", (is_h264 ? "MPEG-4/AVC (H.264)" : "MPEG-2 (H.262)"));
    }

    err = ps_to_ts(ps, output, pad_start, program_repeat, is_h264, input_is_dvd, video_stream,
        audio_stream, want_ac3_audio, want_dolby_as_dvb, pmt_pid, pcr_pid, video_pid, keep_audio,
