...
#define MAX_LINE_LENGTH 80
  ...
  fprint_msg_or_err(is_msg, "%s (%d byte%s):", name, length,
                    (length == 1 ? "" : "s"));
  if (data == nullptr)
    fprint_msg_or_err(is_msg,
                      " <null>"); // Shouldn't happen, but let's be careful.
  else {
...
// ============================================================
static int open_input_as_ES_using_PES(char *name, int quiet,
                                      int force_stream_type, int want_data,
                                      int *is_data, ES_p *es) {
  int err;
...
static int open_input_as_ES_direct(char *name, int quiet, int force_stream_type,
...
int open_input_as_ES(char *name, int use_pes, int quiet, int force_stream_type,
                     int want_data, int *is_data, ES_p *es) {
  if (use_pes)
    return open_input_as_ES_using_PES(name, quiet, force_stream_type, want_data,
                                      is_data, es);
  else
    return open_input_as_ES_direct(name, quiet, force_stream_type, want_data,
                                   is_data, es);
}
    ...
    if (errno == ERANGE && val == 0)
      fprint_err(
          ...
          cmd, arg);
             ...
             (val == LONG_MAX || val == (long unsigned int)LONG_MIN))
 ...
 * explaining will have been written to stderr).
 */
int int_value(const char *prefix, const char *cmd, const char *arg,
...
int connect_socket(char *hostname, int port, int use_tcpip,
  ...
  if (IN_CLASSD(ntohl(ipaddr.sin_addr.s_addr))) {
    ...
    byte ttl = 60;
    result = setsockopt(output, IPPROTO_IP, IP_MULTICAST_TTL, (char *)&ttl,
                        sizeof(ttl));
    if (result < 0) {
    ...
    if (multicast_ifaddr) {
      ...
      result = setsockopt(output, IPPROTO_IP, IP_MULTICAST_IF, (char *)&addr,
                          sizeof(addr));
      if (result < 0) {
