...
static int init_pmt_streams(pmt_p pmt) {
  ...
  pmt->streams =
      (pmt_stream_p)malloc(SIZEOF_PMT_STREAM * PMT_STREAMS_START_SIZE);
 ...
 * occurs.
 */
pmt_p build_pmt(uint16_t program_number, byte version_number,
                uint32_t PCR_pid) {
  pmt_p new2;
...
int add_stream_to_pmt(pmt_p pmt, uint32_t elementary_PID, byte stream_type,
  ...
  if (pmt->num_streams == pmt->streams_size) {
    ...
    pmt->streams =
        (pmt_stream_p)realloc(pmt->streams, newsize * SIZEOF_PMT_STREAM);
    if (pmt->streams == nullptr) {
...
void report_pmt(int is_msg, char *prefix, pmt_p pmt) {
  ...
  if (pmt == nullptr) {
    ...
    fprint_msg_or_err(is_msg, "Program %d, version %d, PCR PID %04x (%d)\n",
                      pmt->program_number, pmt->version_number, pmt->PCR_pid,
                      pmt->PCR_pid);
  ...
  if (pmt->num_streams > 0) {
    ...
    for (ii = 0; ii < pmt->num_streams; ii++) {
      if (prefix != nullptr)
        fprint_msg_or_err(is_msg, prefix);
      fprint_msg_or_err(
          is_msg, "  PID %04x (%4d) -> Stream type %02x (%3d) %s\n",
