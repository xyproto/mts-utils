 ...
 * Returns 0 if it succeeds, 1 if some error occurs.
 */
static int build_avs_frame(avs_context_p context, avs_frame_p *frame,
                           ES_unit_p unit) {
  int err;
  byte *data = unit->data;
  ...
  if (is_avs_frame_item(unit)) {
    ...
    fprint_msg("aspect_ratio=%d, frame_rate_code=%d (%.2f)\n",
               new2->aspect_ratio, new2->frame_rate_code,
               avs_frame_rate(new2->frame_rate_code));
#endif
...
static void _show_item(ES_unit_p unit) {
  ...
  if (is_avs_frame_item(unit))
    fprint_msg("%s frame",
               AVS_PICTURE_CODING_STR(avs_picture_coding_type(unit)));
  else
 ...
 * error occurs.
 */
static int get_next_avs_single_frame(avs_context_p context, int verbose,
                                     avs_frame_p *frame) {
  int err = 0;
 ...
 * error occurs.
 */
int get_next_avs_frame(avs_context_p context, int verbose, int quiet,
                       avs_frame_p *frame) {
  int err;
 ...
 * Returns 0 if it succeeds, 1 if some error occurs.
 */
int write_avs_frame_as_TS(TS_writer_p tswriter, avs_frame_p frame,
                          uint32_t pid) {
  int ii;
  ES_unit_list_p list;
 ...
 * Returns 0 if it succeeds, 1 if some error occurs.
 */
int write_avs_frame_as_TS_with_pts_dts(avs_frame_p frame, TS_writer_p tswriter,
  ...
  for (ii = 0; ii < list->length; ii++) {
    ...
    if (ii == 0)
      err = write_ES_as_TS_PES_packet_with_pts_dts(
          tswriter, unit->data, unit->data_len, video_pid,
          DEFAULT_VIDEO_STREAM_ID, got_pts, pts, got_dts, dts);
    else
      err = write_ES_as_TS_PES_packet(tswriter, unit->data, unit->data_len,
 ...
 * Returns 0 if it succeeds, 1 if some error occurs.
 */
int write_avs_frame_as_TS_with_PCR(avs_frame_p frame, TS_writer_p tswriter,
...
void report_avs_frame(avs_frame_p frame, int report_data) {
  if (frame->is_frame) {
    ...
    fprint_msg(" frame rate %d (%.2f), aspect ratio %d (%s)",
               frame->frame_rate_code, avs_frame_rate(frame->frame_rate_code),
               frame->aspect_ratio,
