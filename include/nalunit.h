 ...
 */
void set_show_nal_reading_details(nal_unit_context_p context, int show) {
...
int build_nal_unit_context(ES_p es, nal_unit_context_p *context) {
  ...
  err = build_param_dict(&new2->pic_param_dict);
  if (err) {
    free_param_dict(&new2->seq_param_dict);
    free(new2);
    return err;
  }
  *context = new2;
 ...
 * Returns 0 if it succeeds, 1 if some error occurs.
 */
static int remove_emulation_prevention(byte data[], int data_len, byte *rbsp[],
                                       int *rbsp_len) {
  int ii;
  int posn = 0;
...
static inline int prepare_rbsp(nal_unit_p nal) {
  ...
  if (nal->rbsp == nullptr) {
    err = remove_emulation_prevention(nal->data, nal->data_len, &(nal->rbsp),
                                      &(nal->rbsp_len));
    if (err) {
 ...
 * Returns 0 if it succeeds, 1 if some error occurs.
 */
static int read_slice_data(nal_unit_p nal, param_dict_p seq_param_dict,
                           param_dict_p pic_param_dict, int show_nal_details) {
  int err;
...
#define CHECK(name)                                                            \
  ...
  if (show_nal_details) {
    fprint_msg("@@ NAL " OFFSET_T_FORMAT_08 "/%04d: size %d\n"
               ...
               nal->data_len, nal->nal_ref_idc, nal->nal_unit_type,
               NAL_UNIT_TYPE_STR(nal->nal_unit_type));
    fprint_msg("   first_mb_in_slice %u, slice_type %u (%s),"
               " pic_parameter_set_id %d\n",
               data->first_mb_in_slice, data->slice_type,
               NAL_SLICE_TYPE_STR(data->slice_type),
 ...
 * Returns 0 if it succeeds, 1 if some error occurs.
 */
static int read_pic_param_set_data(nal_unit_p nal, int show_nal_details) {
  int err;
  bitdata_p bd = nal->bit_data;
 ...
 * Returns 0 if it succeeds, 1 if some error occurs.
 */
static int read_seq_param_set_data(nal_unit_p nal, int show_nal_details) {
  int err;
  bitdata_p bd = nal->bit_data;
...
#define CHECK(name)                                                            \
  ...
  if (show_nal_details) {
    ...
    fprint_msg("   profile_idc %u, constraint set flags: %d %d %d\n",
               data->profile_idc, data->constraint_set0_flag,
               data->constraint_set1_flag, data->constraint_set2_flag);
 ...
 * Returns 0 if it succeeds, 1 if some error occurs
 */
static int read_SEI_recovery_point(nal_unit_p nal, int payloadSize,
...
#define CHECK(name)                                                            \
  ...
  if (show_nal_details) {
    ...
    fprint_msg("   broken_link_flag %d\n   changing_slice_group_idc %d",
               data->broken_link_flag, data->changing_slice_group_idc);
  }
 ...
 * Returns 0 if it succeeds, 1 if some error occurs.
 */
static int read_SEI(nal_unit_p nal, int show_nal_details) {
  int err;
 ...
 *
 *     Caveat: if either `seq_param_dict` or `pic_param_dict` is nullptr, and
 * the NAL unit being interpreted is an IDR or non-IDR unit (i.e., a slice),
 ...
 * Returns 0 if it succeeds, 1 if some error occurs.
 */
static int read_rbsp_data(nal_unit_p nal, param_dict_p seq_param_dict,
                          param_dict_p pic_param_dict, int show_nal_details) {
  int err = 0;
  ...
  if (nal->nal_unit_type == 1 ||
      nal->nal_unit_type == 5) // Coded slice of a (non) IDR picture
    err =
        read_slice_data(nal, seq_param_dict, pic_param_dict, show_nal_details);
  ...
  if (err) {
    fprint_err("### Error reading RBSP data for %s NAL (ref idc %x,"
               ...
               NAL_UNIT_TYPE_STR(nal->nal_unit_type), nal->nal_ref_idc,
               nal->nal_unit_type, nal->unit.start_posn.infile,
               nal->unit.start_posn.inpacket);
  }
 ...
 */
int nal_is_pic_param_set(nal_unit_p nal) { return nal->nal_unit_type == 8; }
 ...
 */
int nal_is_seq_param_set(nal_unit_p nal) { return nal->nal_unit_type == 7; }
...
int nal_is_first_VCL_NAL(nal_unit_p nal, nal_unit_p last) {
  ...
  if (this2->frame_num != that->frame_num)
    nal->start_reason = "Frame number differs";
  else if (this2->field_pic_flag != that->field_pic_flag)
    nal->start_reason = "One is field, the other frame";
  else if (this2->bottom_field_flag_present &&
           that->bottom_field_flag_present &&
           this2->bottom_field_flag != that->bottom_field_flag) {
  ...
  else if (this2->seq_param_set_pic_order_cnt_type == 0 &&
           that->seq_param_set_pic_order_cnt_type == 0 &&
           (this2->pic_order_cnt_lsb != that->pic_order_cnt_lsb ||
            this2->delta_pic_order_cnt_bottom !=
                that->delta_pic_order_cnt_bottom))
    nal->start_reason = "Picture order counts differ";
  else if (this2->seq_param_set_pic_order_cnt_type == 1 &&
           that->seq_param_set_pic_order_cnt_type == 1 &&
           (this2->delta_pic_order_cnt[0] != that->delta_pic_order_cnt[0] ||
            this2->delta_pic_order_cnt[1] != that->delta_pic_order_cnt[1]))
...
#undef SARRAYSIZE
    fprint_msg_or_err(
        is_msg, OFFSET_T_FORMAT_08 "/%04d: %x/%02x %-20s %u (%s) frame %u",
        nal->unit.start_posn.infile, nal->unit.start_posn.inpacket,
        nal->nal_ref_idc, nal->nal_unit_type, what, nal->u.slice.slice_type,
        NAL_SLICE_TYPE_STR(nal->u.slice.slice_type), nal->u.slice.frame_num);
    ...
    fprint_msg_or_err(is_msg, OFFSET_T_FORMAT_08 "/%04d: %x/%02x (%s %u)",
                      ...
                      nal->unit.start_posn.inpacket, nal->nal_ref_idc,
                      nal->nal_unit_type, NAL_UNIT_TYPE_STR(nal->nal_unit_type),
...
static void check_profile(nal_unit_p nal, int show_nal_details) {
  ...
  data = nal->u.seq;
  name = (char *)(data.profile_idc == 66   ? "baseline"
                  : data.profile_idc == 77 ? "main"
                  : data.profile_idc == 88 ? "extended"
                                           : "<unknown>");
  ...
  if (data.profile_idc == 77 || data.constraint_set1_flag == 1)
    ...
    if (sum == 0)
      ...
      if (data.constraint_set0_flag)
        print_err(" baseline");
      if (data.constraint_set1_flag)
        print_err(" main");
      if (data.constraint_set2_flag)
        print_err(" extended");
...
int setup_NAL_data(int verbose, nal_unit_p nal) {
  ...
  if (forbidden_zero_bit) {
    fprint_err("### NAL forbidden_zero_bit is non-zero, at " OFFSET_T_FORMAT
               "/%d\n",
               nal->unit.start_posn.infile, nal->unit.start_posn.inpacket);
...
int write_NAL_unit_as_TS(TS_writer_p tswriter, nal_unit_p nal,
  ...
  int err =
      write_ES_as_TS_PES_packet(tswriter, nal->unit.data, nal->unit.data_len,
...
int build_param_dict(param_dict_p *param_dict) {
  ...
  new2->params =
      (nal_innards_p)malloc(SIZEOF_NAL_INNARDS * NAL_PIC_PARAM_START_SIZE);
  ...
  new2->posns =
      (ES_offset_p)malloc(SIZEOF_ES_OFFSET * NAL_PIC_PARAM_START_SIZE);
  ...
  new2->data_lens =
      (uint32_t *)malloc(sizeof(uint32_t) * NAL_PIC_PARAM_START_SIZE);
  ...
  new2->size = NAL_PIC_PARAM_START_SIZE;
  new2->length = 0;
...
int remember_param_data(param_dict_p param_dict, uint32_t param_id,
  ...
  if (param_dict->length == param_dict->size) {
    ...
    param_dict->ids =
        (int *)realloc(param_dict->ids, newsize * sizeof(uint32_t));
    if (param_dict->ids == nullptr) {
      print_err("### Unable to extend parameter set dictionary array\n");
      return 1;
    }
    param_dict->params = (nal_innards_p)realloc(param_dict->params,
    ...
    param_dict->posns =
        (ES_offset_p)realloc(param_dict->params, newsize * SIZEOF_ES_OFFSET);
    if (param_dict->posns == nullptr) {
      print_err("### Unable to extend parameter set dictionary array\n");
      return 1;
    }
    param_dict->data_lens =
        (uint32_t *)realloc(param_dict->params, newsize * sizeof(uint32_t));
 ...
 */
static inline int lookup_param_data(param_dict_p param_dict, uint32_t param_id,
...
int build_nal_unit_list(nal_unit_list_p *list) {
  ...
  new2->array =
      (nal_unit_p *)malloc(sizeof(nal_unit_p) * NAL_UNIT_LIST_START_SIZE);
...
int append_to_nal_unit_list(nal_unit_list_p list, nal_unit_p nal) {
  if (list->length == list->size) {
    ...
    list->array =
        (nal_unit_p *)realloc(list->array, newsize * sizeof(nal_unit_p));
