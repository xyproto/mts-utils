 ...
 * Returns 0 if it succeeds, 1 if some error occurs.
 */
static int read_slice_data(
    nal_unit_p nal, param_dict_p seq_param_dict, param_dict_p pic_param_dict, int show_nal_details)
{
    int err;
...
#define CHECK(name)                                                                               \
    ...
    if (show_nal_details) {
        fprint_msg("@@ NAL " OFFSET_T_FORMAT_08 "/%04d: size %d\n"
            ...
            nal->unit.start_posn.infile, nal->unit.start_posn.inpacket, nal->data_len,
            nal->nal_ref_idc, nal->nal_unit_type, NAL_UNIT_TYPE_STR(nal->nal_unit_type));
        fprint_msg("   first_mb_in_slice %u, slice_type %u (%s),"
                   " pic_parameter_set_id %d\n",
            data->first_mb_in_slice, data->slice_type, NAL_SLICE_TYPE_STR(data->slice_type),
    ...
    err = get_pic_param_data(pic_param_dict, data->pic_parameter_set_id, &pic_param_data);
    if (err)
        return 1;
    err = get_seq_param_data(
        seq_param_dict, pic_param_data->seq_parameter_set_id, &seq_param_data);
    if (err)
        return 1;
    ...
    if (show_nal_details)
        fprint_msg(
            "   frame_num %u (%d bits)\n", data->frame_num, seq_param_data->log2_max_frame_num);
    ...
    if (seq_param_data->pic_order_cnt_type == 0) {
        err = read_bits(bd, seq_param_data->log2_max_pic_order_cnt_lsb, &data->pic_order_cnt_lsb);
...
#define CHECK(name)                                                                               \
    ...
    if (data->num_slice_groups > 1) {
        ...
        } else if (data->slice_group_map_type == 3 || data->slice_group_map_type == 4
...
#define CHECK(name)                                                                               \
    ...
    if (show_nal_details) {
        ...
        fprint_msg("   profile_idc %u, constraint set flags: %d %d %d\n", data->profile_idc,
            data->constraint_set0_flag, data->constraint_set1_flag, data->constraint_set2_flag);
 ...
 * Returns 0 if it succeeds, 1 if some error occurs
 */
static int read_SEI_recovery_point(nal_unit_p nal, int payloadSize, int show_nal_details)
{
    int err;
    bitdata_p bd = nal->bit_data;
...
#define CHECK(name)                                                                               \
    ...
    if (show_nal_details) {
        ...
        fprint_msg("   recovery_frame_cnt %d\n   exact_match_flag %d\n", data->recovery_frame_cnt,
            data->exact_match_flag);
    ...
    nal_unit_p nal, param_dict_p seq_param_dict, param_dict_p pic_param_dict, int show_nal_details)
{
    ...
    if (err) {
        fprint_err("### Error reading RBSP data for %s NAL (ref idc %x,"
            ...
            NAL_UNIT_TYPE_STR(nal->nal_unit_type), nal->nal_ref_idc, nal->nal_unit_type,
 ...
 */
int nal_is_slice(nal_unit_p nal) { return nal->nal_unit_type == 1 || nal->nal_unit_type == 5; }
...
int nal_is_first_VCL_NAL(nal_unit_p nal, nal_unit_p last)
{
    ...
    if (this2->frame_num != that->frame_num)
    ...
    else if (this2->bottom_field_flag_present && that->bottom_field_flag_present
        ...
        && (this2->pic_order_cnt_lsb != that->pic_order_cnt_lsb
               || this2->delta_pic_order_cnt_bottom != that->delta_pic_order_cnt_bottom))
...
static void check_profile(nal_unit_p nal, int show_nal_details)
{
    ...
    if (data.profile_idc == 77 || data.constraint_set1_flag == 1)
        ...
        print_err("\n");
        fprint_err(
            "Warning: This bitstream declares itself as %s profile (%d)", name, data.profile_idc);
        if (sum == 0)
            print_err(".\n");
        else {
            print_err(",\n");
...
int setup_NAL_data(int verbose, nal_unit_p nal)
{
    ...
    if (forbidden_zero_bit) {
        fprint_err("### NAL forbidden_zero_bit is non-zero, at " OFFSET_T_FORMAT "/%d\n",
            nal->unit.start_posn.infile, nal->unit.start_posn.inpacket);
...
int find_next_NAL_unit(nal_unit_context_p context, int verbose, nal_unit_p* nal)
{
    ...
    err = read_rbsp_data(
        *nal, context->seq_param_dict, context->pic_param_dict, context->show_nal_details);
    if (err) {
        free_nal_unit(nal);
        return 2;
    }
 ...
 */
static inline int lookup_param_data(
    param_dict_p param_dict, uint32_t param_id, nal_innards_p* param_data)
{
    int ii;
    for (ii = 0; ii < param_dict->length; ii++) {
        if (param_dict->ids[ii] == param_id) {
