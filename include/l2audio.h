...
static const int bitrate_v1l1[] = {0,   32,  64,  96,  128, 160, 192, 224,
                                   256, 288, 320, 352, 384, 416, 448, 0};
static const int bitrate_v1l2[] = {0,   32,  48,  56,  64,  80,  96,  112,
                                   128, 160, 192, 224, 256, 320, 384, 0};
static const int bitrate_v1l3[] = {0,   32,  40,  48,  56,  64,  80,  96,
                                   112, 128, 160, 192, 224, 256, 320, 0};
static const int bitrate_v2l1[] = {0,   32,  48,  56,  64,  80,  96,  112,
                                   128, 144, 160, 176, 192, 224, 256, 0};
static const int bitrate_v2l2[] = {0,  8,  16, 24,  32,  40,  48,  56,
...
const unsigned int aud_frame_rate_n[16] = {
    AUD_FRAME_RATE_N_0,  AUD_FRAME_RATE_N_1,  AUD_FRAME_RATE_N_2,
    AUD_FRAME_RATE_N_3,  AUD_FRAME_RATE_N_4,  AUD_FRAME_RATE_N_5,
    AUD_FRAME_RATE_N_6,  AUD_FRAME_RATE_N_7,  AUD_FRAME_RATE_N_8,
    AUD_FRAME_RATE_N_9,  AUD_FRAME_RATE_N_10, AUD_FRAME_RATE_N_11,
    AUD_FRAME_RATE_N_12, AUD_FRAME_RATE_N_13, AUD_FRAME_RATE_N_14,
    AUD_FRAME_RATE_N_15};
...
#define JUST_ENOUGH 6 // just enough to hold the bits of the headers we want
  ...
  while (header[0] != 0xFF || (header[1] & 0xe0) != 0xe0) {
    ...
    fprint_err(
        ...
        (header[0] & 0xF0) >> 4, (header[0] & 0x0F), (header[1] & 0xe0) >> 4);
  ...
  frame_length =
      peek_frame_header((header[1] << 16) | (header[2] << 8) | header[3]);
  if (frame_length < 1) {
  ...
  if (data == nullptr) {
    print_err(
        "### Unable to extend data buffer for MPEG layer 2 audio frame\n");
    free(data);
    return 1;
  }
  ...
  if (err) {
    if (err == EOF)
      print_err(
          "### Unexpected EOF reading rest of MPEG layer 2 audio frame\n");
    else
