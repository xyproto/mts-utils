 ...
 */
int rewind_program_stream(PS_reader_p ps) { return seek_using_PS_reader(ps, ps->start); }
 ...
 */
static int read_PS_bytes(PS_reader_p ps, int num_bytes, byte* buffer, offset_t* posn)
{
    int err;
    int offset = 0;
...
int read_PS_pack_header_body(PS_reader_p ps, PS_pack_header_p hdr)
{
    ...
    if ((hdr->data[0] & 0xF0) == 0x20) {
        ...
        hdr->scr = (((uint64_t)(hdr->data[0] & 0x09)) << 29) | (((uint64_t)hdr->data[1]) << 22)
            | (((uint64_t)(hdr->data[2] & 0xFE)) << 14) | (((uint64_t)hdr->data[3]) << 7)
            | (((uint64_t)(hdr->data[4] & 0xFE)) >> 1);
        hdr->program_mux_rate = (((uint32_t)(hdr->data[5] & 0x7F)) << 15)
            | (((uint32_t)hdr->data[6]) << 7) | (((uint32_t)(hdr->data[7] & 0xFE)) >> 1);
        ...
        hdr->scr_base = (((uint64_t)(hdr->data[0] & 0x38)) << 27)
            ...
            | (((uint64_t)(hdr->data[2] & 0xF8)) << 12) | (((uint64_t)(hdr->data[2] & 0x03)) << 13)
            | (((uint64_t)hdr->data[3]) << 5) | (((uint64_t)(hdr->data[4] & 0xF8)) >> 3);
        hdr->scr_extn = (((uint32_t)(hdr->data[4] & 0x03)) << 7) | (((uint32_t)hdr->data[5]) >> 1);
        hdr->scr = hdr->scr_base * 300 + hdr->scr_extn;
        hdr->program_mux_rate = (((uint32_t)hdr->data[6] << 14)) | (((uint32_t)hdr->data[7] << 6))
            | (((uint32_t)hdr->data[8] >> 2));
        hdr->pack_stuffing_length = hdr->data[9] & 0x07;
    }

#if DEBUG // XXX
    fprint_msg("Pack header body: scr_base " LLU_FORMAT ", scr_extn %u, scr " LLU_FORMAT
               ", mux rate %u, stuffing %d\n",
        hdr->scr_base, hdr->scr_extn, hdr->scr, hdr->program_mux_rate, hdr->pack_stuffing_length);
#endif
...
static inline void determine_ac3_details(byte* data, bool verbose, byte* bsmod, byte* acmod)
{
    ...
    *acmod = (data[6] & 0xC0) >> 6;
    if (verbose) {
        ...
        fprint_msg("    bsid        %x (%s)\n", bsid,
            (bsid == 8 ? "standard"
                       : bsid == 6 ? "A52a alternate" : bsid < 8 ? "standard subset" : "???"));
    ...
    *acmod = 0;
    ...
    if (is_dvd) {
        ...
        if (what == SUBSTREAM_AC3 || what == SUBSTREAM_DTS) {
            ...
            } else if (what == SUBSTREAM_AC3 && !(sub_data[0] == 0x0B && sub_data[1] == 0x77)) {
                fprint_msg("*** expected AC3 sync 0x0B77, but found 0x%02x%02x\n", sub_data[0],
                    sub_data[1]);
                what = SUBSTREAM_ERROR;
            } else if (what == SUBSTREAM_DTS
                && !(sub_data[0] == 0x7F && sub_data[1] == 0xFE && sub_data[2] == 0x80
                       && sub_data[3] == 0x01)) {
    ...
    int* num_video_written, bool verbose, bool quiet)
{
    ...
    if (*num_video_written == 0) {
        ...
        // the new video (!)
        err = write_pat_and_pmt(output, prog_data->transport_stream_id, prog_data->prog_list,
            prog_data->pmt_pid, prog_data->pmt);
        if (err) {
 ...
 * Returns 0 if all went well, 1 if something went wrong.
 */
static int write_DVD_AC3_data(
    TS_writer_p output, struct PS_packet* packet, struct program_data* prog_data)
{
...
#if DEBUG_AC3
    fprint_msg(".. frame_count=%d, offset=%4d, got_pts=%d ", frame_count, offset, got_pts);
    ...
    if (frame_count == 0 || offset <= 1 || !got_pts) {
        ...
        fprint_msg("write first %4d bytes, then move data down %4d, leaving %4d\n", offset - 1,
            3 + offset, data_len - 3 - offset);
#endif

        // First, the part before the first packet...
        err = write_ES_as_TS_PES_packet(
            output, data + 4, offset - 1, prog_data->audio_pid, PRIVATE1_AUDIO_STREAM_ID);
        if (err)
            return 1;
    ...
    bool quiet)
{
    ...
    if (prog_data->audio_stream == -1) // take the first audio stream we find...
    {
        if (stream_id == PRIVATE1_AUDIO_STREAM_ID && is_h222_pes) {
            ...
            int what = identify_private1_data(
                packet, prog_data->is_dvd, verbose, &substream_index, &bsmod, &asmod);
            if (what != SUBSTREAM_AC3) {
        ...
        && prog_data->audio_stream == PRIVATE1_AUDIO_STREAM_ID && is_h222_pes) {
...
#define MAX_IGNORED_NON_AC3 10 // report the first 10 substreams we're ignoring
                ...
                for (ii = 0; ii < MAX_IGNORED_NON_AC3; ii++) {
                    if (ignored_non_AC3[ii] == lookfor)
                        ...
                        fprint_msg("Ignoring %sprivate_stream_1 substream 0x%x (%d)"
                                   " containing %s\n",
                            (SUBSTREAM_IS_AUDIO(what) ? "" : "non-audio "), substream_index,
                            substream_index, SUBSTREAM_STR(what));
                        break;
                    }
                }
            }
    ...
    if (prog_data->is_dvd && stream_id == PRIVATE1_AUDIO_STREAM_ID && is_h222_pes) {
    ...
    for (;;) {
        ...
        while (stream_id != 0xba) // i.e., until the start of the next pack
        {
            ...
            if (IS_AUDIO_STREAM_ID(stream_id)) {
                if (keep_audio) {
                    err = write_audio(output, stream_id, &packet, prog_data, &num_audio_ignored,
                        &num_audio_written, verbose, quiet);
                    if (err) {
                        fprint_err(
                            "### Error writing audio packet at " OFFSET_T_FORMAT " to TS\n", posn);
                        return 1;
                    }
                }
                ...
                if (err) {
                    fprint_err(
                        "### Error writing video packet at " OFFSET_T_FORMAT " to TS\n", posn);
                    return 1;
                }
            } else if (verbose) {
...
#define PROGRAM_NUMBER 1
    ...
    err = _ps_to_ts(
        ps, output, &prog_data, pad_start, program_repeat, keep_audio, max, verbose, quiet);
    if (err) {
        free_pidint_list(&prog_data.prog_list);
