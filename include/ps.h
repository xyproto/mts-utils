...
int build_PS_reader(int input, int quiet, PS_reader_p *ps) {
  ...
  err = find_PS_pack_header_start(new2, false, PACK_HEADER_SEARCH_DISTANCE,
                                  &(new2->start));
  if (err) {
  ...
  if (input != STDIN_FILENO) {
    ...
    err = seek_using_PS_reader(new2, new2->start);
    if (err) {
 ...
 */
int seek_using_PS_reader(PS_reader_p ps, offset_t posn) {
 ...
 */
int rewind_program_stream(PS_reader_p ps) {
 ...
 */
static int read_PS_bytes(PS_reader_p ps, int num_bytes, byte *buffer,
                         offset_t *posn) {
  int err;
  int offset = 0;
 ...
 * if some other error occurs. `packet->data` will be nullptr if EOF is
 * returned.
 */
 ...
 */
int read_PS_pack_header_body(PS_reader_p ps, PS_pack_header_p hdr) {
  int err;
  ...
  if ((hdr->data[0] & 0xF0) == 0x20) {
    ...
    hdr->scr_extn = (((uint32_t)(hdr->data[4] & 0x03)) << 7) |
                    (((uint32_t)hdr->data[5]) >> 1);
    hdr->scr = hdr->scr_base * 300 + hdr->scr_extn;
    hdr->program_mux_rate = (((uint32_t)hdr->data[6] << 14)) |
...
#if DEBUG // XXX
  fprint_msg("Pack header body: scr_base " LLU_FORMAT
             ", scr_extn %u, scr " LLU_FORMAT ", mux rate %u, stuffing %d\n",
...
#if DEBUG
  fprint_msg("Packet at " OFFSET_T_FORMAT ", stream id %02X (", *posn,
             *stream_id);
 ...
 */
static inline void determine_ac3_details(byte *data, int verbose, byte *bsmod,
                                         byte *acmod) {
...
int identify_private1_data(struct PS_packet *packet, int is_dvd, int verbose,
  ...
  *acmod = 0;
  ...
  if (is_dvd) {
      ...
      if (sub_data >= packet->data + packet_length) // leave off the + 6
      {
        ...
        if (verbose)
          fprint_msg("*** expected %s, but data at %p is beyond"
                     " packet->end at %p\n",
                     (what == SUBSTREAM_DTS ? "DTS" : "AC3"), sub_data,
                     packet->data + 6 + packet_length);
    ...
    } else if (data[0] == 0x7F && data[1] == 0xFE && data[2] == 0x80 &&
               data[3] == 0x01)
      what = SUBSTREAM_DTS;
  }
  if (verbose) {
 ...
 */
static int write_video(TS_writer_p output, struct PS_pack_header *header,
                       byte stream_id, struct PS_packet *packet,
                       struct program_data *prog_data, int *num_video_ignored,
  ...
  if (*num_video_written == 0) {
    ...
    err = add_stream_to_pmt(prog_data->pmt, prog_data->video_pid,
                            prog_data->video_type, 0, nullptr);
    if (err)
      return 1;
    ...
    // the new video (!)
    err = write_pat_and_pmt(output, prog_data->transport_stream_id,
                            prog_data->prog_list, prog_data->pmt_pid,
                            prog_data->pmt);
    if (err) {
  ...
  err = write_PES_as_TS_PES_packet(
      output, packet->data, packet->data_len, prog_data->video_pid,
 ...
 * Returns 0 if all went well, 1 if something went wrong.
 */
static int write_DVD_AC3_data(TS_writer_p output, struct PS_packet *packet,
...
#if DEBUG_AC3
  fprint_msg(".. frame_count=%d, offset=%4d, got_pts=%d ", frame_count, offset,
             got_pts);
  ...
  if (frame_count == 0 || offset <= 1 || !got_pts) {
    ...
    err = write_PES_as_TS_PES_packet(output, packet->data, packet->data_len - 4,
                                     prog_data->audio_pid,
                                     PRIVATE1_AUDIO_STREAM_ID, false, 0, 0);
    if (err)
      return 1;
  } else {
    ...
    // And we then have something suitable for outputting as-is
    err = write_PES_as_TS_PES_packet(
        output, packet->data, packet->data_len - 3 - offset,
 ...
 * Returns 0 if all went well, 1 if something went wrong.
 */
static int write_audio(TS_writer_p output, byte stream_id,
                       struct PS_packet *packet, struct program_data *prog_data,
                       int *num_audio_ignored, int *num_audio_written,
  ...
  if (prog_data->audio_stream == -1) // take the first audio stream we find...
  {
    ...
    (*num_audio_ignored)++;
    return 0;
  } else if (prog_data->is_dvd && stream_id == PRIVATE1_AUDIO_STREAM_ID &&
...
#define MAX_IGNORED_NON_AC3 10 // report the first 10 substreams we're ignoring
  ...
  if (prog_data->is_dvd && stream_id == PRIVATE1_AUDIO_STREAM_ID &&
    ...
    err = write_PES_as_TS_PES_packet(
        output, packet->data, packet->data_len, prog_data->audio_pid,
        (prog_data->want_ac3 ? PRIVATE1_AUDIO_STREAM_ID
 ...
 * - `output_dolby_as_dvb` should be true if Dolby (AC-3) audio (if selected)
 * should be output using the DVB stream type 0x06, false if using the ATSC
 * stream type 0x81. This is ignored if the audio being output is not Dolby.
 ...
 * Returns 0 if all went well, 1 if something went wrong.
 */
static int _ps_to_ts(PS_reader_p ps, TS_writer_p output,
                     struct program_data *prog_data, int pad_start,
                     int program_repeat, int keep_audio, int max, int verbose,
                     int quiet) {
  int ii, err;
  ...
  if (!quiet)
    fprint_msg(
        "Writing transport stream id 1, PMT PID 0x%02x, PCR PID 0x%02x\n",
...
#define READ_NEXT_PS_PACKET_START                                              \
  ...
  for (;;) {
    ...
    if (stream_id == 0xbb) {
      ...
      if (err) {
        fprint_err(
            "### Error reading system header starting at " OFFSET_T_FORMAT "\n",
            posn);
        return 1;
      }
      num_system_headers++;
      ...
      if (IS_AUDIO_STREAM_ID(stream_id)) {
        if (keep_audio) {
          err = write_audio(output, stream_id, &packet, prog_data,
                            &num_audio_ignored, &num_audio_written, verbose,
                            quiet);
          if (err) {
            fprint_err("### Error writing audio packet at " OFFSET_T_FORMAT
                       " to TS\n",
                       posn);
            return 1;
          }
        ...
        err =
            write_video(output, &header, stream_id, &packet, prog_data,
...
int ps_to_ts(PS_reader_p ps, TS_writer_p output, int pad_start,
             ...
             int audio_stream, int want_ac3_audio, int output_dolby_as_dvb,
