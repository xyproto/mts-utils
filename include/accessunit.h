 ...
 * Returns 0 if it succeeds, 1 if some error occurs.
 */
static int access_unit_append(access_unit_p access_unit, nal_unit_p nal,
                              int starts_primary, nal_unit_list_p pending) {
  int err;
  ...
  for (ii = 0; ii < access_unit->nal_units->length; ii++) {
    ...
    if (ii == 0)
      err = write_ES_as_TS_PES_packet_with_pts_dts(
          tswriter, nal->unit.data, nal->unit.data_len, video_pid,
          DEFAULT_VIDEO_STREAM_ID, got_pts, pts, got_dts, dts);
    else
      err = write_ES_as_TS_PES_packet(tswriter, nal->unit.data,
                                      nal->unit.data_len, video_pid,
...
int write_access_unit_as_TS_with_PCR(access_unit_p access_unit,
  ...
  for (ii = 0; ii < access_unit->nal_units->length; ii++) {
    ...
    if (ii == 0)
      err = write_ES_as_TS_PES_packet_with_pcr(
          tswriter, nal->unit.data, nal->unit.data_len, video_pid,
          DEFAULT_VIDEO_STREAM_ID, pcr_base, pcr_extn);
    else
      err = write_ES_as_TS_PES_packet(tswriter, nal->unit.data,
                                      nal->unit.data_len, video_pid,
                                      DEFAULT_VIDEO_STREAM_ID);
...
static inline int end_access_unit(access_unit_context_p context,
                                  access_unit_p access_unit, int show_details) {
  if (show_details) {
...
int build_access_unit_context(ES_p es, access_unit_context_p *context) {
  int err;
  access_unit_context_p new2 =
      (access_unit_context_p)malloc(SIZEOF_ACCESS_UNIT_CONTEXT);
  if (new2 == nullptr) {
...
static int maybe_remember_access_unit(reverse_data_p reverse_data,
                                      access_unit_p access_unit, int verbose) {
  ...
  if (access_unit->primary_start != nullptr &&
    ...
    if (verbose)
      fprint_msg("REMEMBER IDR %5d at " OFFSET_T_FORMAT_08 "/%04d for %5d\n",
                 access_unit->index, start_posn.infile, start_posn.inpacket,
                 num_bytes);
  }
  return 0;
}
...
int get_next_access_unit(access_unit_context_p context, int quiet,
  ...
  if (context->pending_nal != nullptr) {
    err = access_unit_append(access_unit, context->pending_nal, true,
    ...
    if (nal_is_slice(nal)) {
      if (!access_unit->started_primary_picture) {
        ...
        if (access_unit->started_primary_picture) {
          ...
          err =
              access_unit_append(access_unit, nal, true, context->pending_list);
          if (err)
            goto give_up_free_nal;
          reset_nal_unit_list(context->pending_list, false);
        }
        ...
        err =
            access_unit_append(access_unit, nal, false, context->pending_list);
        if (err)
          goto give_up_free_nal;
        reset_nal_unit_list(context->pending_list, false);
      }
      ...
      if (access_unit->started_primary_picture) {
        ...
        if (err)
          goto give_up_free_nal;
               ...
               nal->nal_unit_type == 13 || nal->nal_unit_type == 14 ||
               nal->nal_unit_type == 15 || nal->nal_unit_type == 16 ||
 ...
 * some error occurs.
 */
static int get_next_field_of_pair(access_unit_context_p context, int quiet,
                                  int show_details, int first_time,
                                  access_unit_p *access_unit) {
  int err;
  ...
  if (second->field_pic_flag == 0) {
    ...
    *access_unit = second;
    ...
    free_access_unit(access_unit);
    *access_unit = second;
    err = get_next_field_of_pair(context, quiet, show_details, false,
                                 access_unit);
    if (err)
      return 1;
  } else {
...
int get_next_h264_frame(access_unit_context_p context, int quiet,
  ...
  *frame = nullptr;
  ...
  if (access_unit->field_pic_flag == 1) {
    ...
    err = get_next_field_of_pair(context, quiet, show_details, true,
                                 &access_unit);
    if (err) {
      free_access_unit(&access_unit);
      return 1;
