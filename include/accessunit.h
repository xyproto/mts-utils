 ...
 */
static inline int build_access_unit(access_unit_p* acc_unit, uint32_t index)
{
    int err;
 ...
 */
void report_access_unit(access_unit_p access_unit)
{
    int ii;
    ...
    if (access_unit->field_pic_flag)
        fprint_msg("  %s field of frame %u\n",
            (access_unit->bottom_field_flag == 1 ? "Bottom" : "Top"), access_unit->frame_num);
    else
 ...
 * Returns 0 if it succeeds, 1 if some error occurs.
 */
static int access_unit_append(
    access_unit_p access_unit, nal_unit_p nal, int starts_primary, nal_unit_list_p pending)
{
    int err;
 ...
 * Returns 0 if it succeeds, 1 if some error occurs.
 */
static int merge_access_unit_nals(access_unit_p access_unit1, access_unit_p* access_unit2)
{
    int err, ii;

    for (ii = 0; ii < (*access_unit2)->nal_units->length; ii++) {
        err = append_to_nal_unit_list(
            access_unit1->nal_units, (*access_unit2)->nal_units->array[ii]);
        if (err) {
    ...
    access_unit1->ignored_broken_NAL_units += (*access_unit2)->ignored_broken_NAL_units;
 ...
 * Returns 0 if it succeeds, 1 if some error occurs.
 */
int write_access_unit_as_ES(access_unit_p access_unit, access_unit_context_p context, FILE* output)
{
    int ii, err;
    for (ii = 0; ii < access_unit->nal_units->length; ii++) {
 ...
 * Returns 0 if it succeeds, 1 if some error occurs.
 */
static int write_access_unit_trailer_as_TS(
    access_unit_context_p context, TS_writer_p tswriter, uint32_t video_pid)
{
    int err;
...
int write_access_unit_as_TS_with_pts_dts(access_unit_p access_unit, access_unit_context_p context,
    TS_writer_p tswriter, uint32_t video_pid, int got_pts, uint64_t pts, int got_dts, uint64_t dts)
{
    int ii, err;
    ...
    for (ii = 0; ii < access_unit->nal_units->length; ii++) {
        ...
        if (ii == 0)
            err = write_ES_as_TS_PES_packet_with_pts_dts(tswriter, nal->unit.data,
                nal->unit.data_len, video_pid, DEFAULT_VIDEO_STREAM_ID, got_pts, pts, got_dts,
                dts);
        else
            err = write_ES_as_TS_PES_packet(
                tswriter, nal->unit.data, nal->unit.data_len, video_pid, DEFAULT_VIDEO_STREAM_ID);
    ...
    TS_writer_p tswriter, uint32_t video_pid, uint64_t pcr_base, uint32_t pcr_extn)
{
    ...
    for (ii = 0; ii < access_unit->nal_units->length; ii++) {
        ...
        if (ii == 0)
            err = write_ES_as_TS_PES_packet_with_pcr(tswriter, nal->unit.data, nal->unit.data_len,
 ...
 * Returns 0 if it succeeds, 1 if some error occurs.
 */
static inline int end_access_unit(
    access_unit_context_p context, access_unit_p access_unit, int show_details)
{
    if (show_details) {
...
int build_access_unit_context(ES_p es, access_unit_context_p* context)
{
    ...
    err = build_nal_unit_list(&new2->pending_list);
    if (err) {
        print_err("### Error building access unit context datastructure\n");
        free_nal_unit_context(&new2->nac);
        free(new2);
        return err;
    }
 ...
 */
void reset_access_unit_context(access_unit_context_p context)
{
 ...
 * Returns 0 if it succeeds, 1 if some error occurs.
 */
static int remember_earlier_primary_start(access_unit_context_p context, nal_unit_p nal)
{
    ...
    reverse_data_p reverse_data, access_unit_p access_unit, int verbose)
{
    ...
    if (access_unit->primary_start != NULL && access_unit->primary_start->nal_ref_idc != 0
        ...
        err = remember_reverse_h264_data(reverse_data, access_unit->index, start_posn, num_bytes);
        if (err) {
            fprint_err("### Error remembering access unit %d for reversing\n", access_unit->index);
            return 1;
        }
        if (verbose)
            fprint_msg("REMEMBER IDR %5d at " OFFSET_T_FORMAT_08 "/%04d for %5d\n",
                access_unit->index, start_posn.infile, start_posn.inpacket, num_bytes);
    }
    return 0;
}
    ...
    access_unit_context_p context, int quiet, int show_details, access_unit_p* ret_access_unit)
{
    ...
    for (;;) {
        ...
        if (nal_is_slice(nal)) {
            ...
            || nal->nal_unit_type == NAL_PIC_PARAM_SET || nal->nal_unit_type == 13
            || nal->nal_unit_type == 14 || nal->nal_unit_type == 15 || nal->nal_unit_type == 16
            || nal->nal_unit_type == 17 || nal->nal_unit_type == 18) {
        ...
        *access_unit = second;
        ...
        if (!quiet)
            fprint_err("!!! Field with frame number %d (%x) followed by"
                ...
                (*access_unit)->frame_num, (*access_unit)->frame_num, second->frame_num,
                second->frame_num);
    ...
    access_unit_context_p context, int quiet, int show_details, access_unit_p* frame)
{
    ...
    *frame = NULL;
    ...
    if (context->reverse_data) {
        err = maybe_remember_access_unit(context->reverse_data, access_unit, show_details);
        if (err) {
            free_access_unit(&access_unit);
            return 1;
        }
