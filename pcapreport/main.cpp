...
struct pcapreport_stream_struct {
  ...
  int seen_bad;
  int seen_dodgy; // Count of packets that we aren't completely happy with but
                  // have declared good
...
static unsigned int jitter_value(const jitter_env_t *const je) {
  return je->max_val - je->min_val;
}

static unsigned int jitter_add(jitter_env_t *const je, const int delta,
                               const uint32_t time, const uint32_t range) {
  jitter_el_t *const eob = je->buf + JITTER_BUF_SIZE;
  jitter_el_t *const in_el = je->buf + je->in_n;
  jitter_el_t *out_el = je->buf + je->out_n;
  jitter_el_t *const next_el =
      (je->in_n == JITTER_BUF_SIZE - 1) ? je->buf : in_el + 1;
...
static void jitter_clear(jitter_env_t *const je) {
  je->in_n = 0;
  je->out_n = 0;
  je->max_val = 0;
  je->min_val = 0;
...
static uint64_t pkt_time(const pcaprec_hdr_t *const pcap_pkt_hdr) {
  return (((int64_t)pcap_pkt_hdr->ts_usec * 9) / 100) +
         ((int64_t)pcap_pkt_hdr->ts_sec * 90000);
}

static char *vlan_name(const char *prefix, const pcapreport_stream_t *const st,
                       const size_t blen, char *const buf) {
  if (st->vlan_count == 0) {
    buf[0] = '\0';
  } else {
    int i;
...
static char *section_name(const pcapreport_ctx_t *const ctx,
                          const pcapreport_stream_t *const st, char *const pbuf,
                          const size_t pbuf_len) {
  if (!ctx->file_split_section) {
    *pbuf = '\0';
    return pbuf;
  }

  snprintf(pbuf, pbuf_len, "_S%d",
           st->section_last == nullptr ? 0 : st->section_last->section_no);
  return pbuf;
}

static void stream_gen_names2(const pcapreport_ctx_t *const ctx,
  ...
  if (ctx->filter_dest_addr == 0 || ctx->filter_dest_port == 0) {
    snprintf(identifier, 90, "%s_%u.%u.%u.%u_%u%s",
             vlan_name("_V", st, sizeof(pbuf), pbuf), dest_addr >> 24,
             (dest_addr >> 16) & 0xff, (dest_addr >> 8) & 0xff,
             dest_addr & 0xff, dest_port,
  ...
  if (ctx->extract) {
    ...
    if (!fixed_extract_name)
      snprintf(name + base_len, 100, "%s%s%s", identifier, ".",
               (rtp_header != nullptr && rtp_header->is_rtp_raw) ? "rtp"
                                                                 : "ts");
    st->output_name = name;
  }
...
static void stream_gen_names(const pcapreport_ctx_t *const ctx,
                             pcapreport_stream_t *const st,
                             const rtp_header_t *const rtp_header) {
...
static void stream_close_files(const pcapreport_ctx_t *const ctx,
                               pcapreport_stream_t *const st) {
  if (st->output_file != nullptr) {
    if (st->seen_dodgy != 0) {
      fprint_msg(">%d> WARNING: %d dodgy packet%s written to: %s\n",
                 st->stream_no, st->seen_dodgy, st->seen_dodgy == 1 ? "" : "s",
                 st->output_name);
    }
    if (st->seen_bad != 0) {
      fprint_msg(">%d> WARNING: %d bad packet%s excluded from: %s\n",
                 st->stream_no, st->seen_bad, st->seen_bad == 1 ? "" : "s",
                 st->output_name);
    }
...
section_create(const pcapreport_ctx_t *const ctx, pcapreport_stream_t *const st,
               const pcaprec_hdr_t *const pcap_pkt_hdr) {
  pcapreport_section_t *const tsect =
      (pcapreport_section_t *)calloc(1, sizeof(*tsect));
  ...
  tsect->time_final = tsect->time_start = pkt_time(pcap_pkt_hdr);
...
static int digest_times_read(void *handle, byte *out_buf, size_t len) {
  ...
  memmove(st->tmp_buf, &st->tmp_buf[nr_bytes], new_tmp_len);
...
// 33 bit comparison
static int64_t pts_diff(const uint64_t a, const uint64_t b) {
  return ((int64_t)(a - b) << 31) >> 31;
}

static int64_t pts_diff_abs(const uint64_t a, const uint64_t b) {
...
static int digest_times(pcapreport_ctx_t *const ctx,
                        ...
                        const rtp_header_t *const rtp_header,
                        const byte *const data, const uint32_t len) {
  int rv;
  unsigned int rtp_seq_delta = 0;

  // Deal with RTP contents - currently held with stream but could be moved to
  // section especially if we do more timestamp analysis
  if (rtp_header->is_rtp_ts) {
    ...
    if (ri->ssrc != rtp_header->ssrc && ri->n != 0 && !ri->multiple_ssrc) {
      fprint_msg("!%d! Multiple SSRCs detected: SSRCs: %u,%u,...\n",
                 st->stream_no, ri->ssrc, rtp_header->ssrc);
    ...
    if (rtp_seq_delta != 0) {
      fprint_msg("!%d! @%u: RTP seq delta (%u->%u) != 1\n", st->stream_no,
                 ctx->pkt_counter, ri->last_seq, rtp_header->sequence_number);
    }
  ...
  if (st->ts_r == nullptr) {
    rv = build_TS_reader_with_fns(st, digest_times_read, digest_times_seek,
                                  &st->ts_r);
    if (rv) {
  ...
  while (1) {
    ...
    // Right. Split it ..
    {
      ...
      rv = split_TS_packet(pkt, &pid, &pusi, &adapt, &adapt_len, &payload,
                           &payload_len);
      if (rv) {
        fprint_msg(">%d> WARNING: TS packet %d [ packet %d @ %d.%d s ] cannot "
                   "be split.\n",
                   st->stream_no, st->ts_counter, ctx->pkt_counter,
        ...
        if (adapt && adapt_len) {
          ...
          get_PCR_from_adaptation_field(adapt, adapt_len, &has_pcr, &pcr);
          ...
          if (has_pcr) {
            ...
            if (pid != st->pcr_pid) {
              ...
              skew = tsect->pcr_count == 0
                         ? 0LL
                         : pcr_time_offset -
                               pts_diff(tsect->time_first, tsect->pcr_start);
              ...
              if (st->skew_discontinuity_threshold > 0 &&
                ...
                const int64_t time_delta =
                    pts_diff_abs(t_pcr, tsect->time_last);
              ...
              if (tsect->pcr_count == 0) {
                if (tsect->section_no != 0) {
                  fprint_msg(">%d> Skew discontinuity! Skew = %lld (> %lld) at"
                             ...
                             st->stream_no, skew,
                             st->skew_discontinuity_threshold, st->ts_counter,
                             ctx->pkt_counter, pcr, pcap_pkt_hdr->ts_sec,
                             pcap_pkt_hdr->ts_usec);
                }

                tsect->pkt_final = ctx->pkt_counter;
                tsect->pcr_last = tsect->pcr_start = pcr;
                tsect->time_last = tsect->time_first = t_pcr;
              ...
              if (rtp_header->is_rtp_ts) {
                ...
                int32_t rtp_skew = (int32_t)(rtp_header->timestamp -
                                             (uint32_t)(t_pcr & 0xffffffffU));
              ...
              if (ctx->time_report) {
                ...
                double skew_rate =
                    (rel_tim == 0)
                        ? 0.0
                        : (double)skew /
                              ((double)((double)rel_tim / (60 * 90000)));

                fprint_msg(
                    ">%d> [ts %d net %d ] PCR %lld Time %d.%d [rel %d.%d]  - "
                    "skew "
                    "= %lld (delta = %lld, rate = %.4g PTS/min) - jitter=%u\n",
                    st->stream_no, st->ts_counter, ctx->pkt_counter, pcr,
                    pcap_pkt_hdr->ts_sec, pcap_pkt_hdr->ts_usec,
                    (int)(rel_tim / (int64_t)1000000), (int)rel_tim % 1000000,
                    skew, pcr_time_offset - st->last_time_offset, skew_rate,
                    cur_jitter);
              }
              ...
              if (st->csv_name != nullptr) // We should be outputting to file
              {
                ...
                fprintf(st->csv_file,
                        "%d," LLU_FORMAT "," LLU_FORMAT "," LLD_FORMAT ",%u\n",
...
static int write_out_packet(pcapreport_ctx_t *const ctx,
                            pcapreport_stream_t *const st, const byte *data,
                            const uint32_t len) {
  int rv;
  ...
  if (st->output_name) {
    if (st->output_file == nullptr) {
      fprint_msg("pcapreport: Dumping %s packets for %s:%d to %s\n",
                 ctx->good_ts_only ? "good ts"
                 : ctx->keep_bad   ? "all"
                                   : "ts",
                 ipv4_addr_to_string(st->output_dest_addr),
                 st->output_dest_port, st->output_name);
    ...
    if (rv != pkts) {
      fprint_err("### pcapreport: Couldn't write %d bytes"
                 " to %s (error = %d).\n",
                 len, st->output_name, ferror(st->output_file));
      return 1;
    }
  }
  return 0;
...
static int stream_ts_check(const pcapreport_ctx_t *const ctx,
                           pcapreport_stream_t *const st,
                           const byte *const data, const uint32_t len) {
  const byte *ptr;
  int good = 0;
    ...
    // need header
    {
      ...
      if (rv != 1) {
        fprint_err("### pcapreport: Couldn't write RTP hdr bytes"
                   " to %s (error = %d).\n",
                   st->output_name, ferror(st->output_file));
        return 1;
      }
    }
...
static int stream_rtp_check(const pcapreport_ctx_t *const ctx,
                            pcapreport_stream_t *const st,
                            const byte *const data, const uint32_t len,
...
static void stream_merge_vlan_info(pcapreport_stream_t *const st,
                                   const ethernet_packet_t *const epkt) {
  int i;
...
void stream_close(pcapreport_ctx_t *const ctx, pcapreport_stream_t **pst) {
...
stream_create(pcapreport_ctx_t *const ctx,
              const pcaprec_hdr_t *const pcap_pkt_hdr,
              const ethernet_packet_t *const epkt, uint32_t const dest_addr,
...
static char *map_to_string(unsigned int n, const size_t blen, char *const buf) {
  int i = 0;
  char *p = buf;
  char *const eob = buf + blen;
  int first = true;
...
static void stream_analysis(const pcapreport_ctx_t *const ctx,
                            const pcapreport_stream_t *const st) {
  ...
  fprint_msg("Stream %d: Dest:%s %u.%u.%u.%u:%u\n", st->stream_no,
             vlan_name(" VLAN:", st, sizeof(pbuf), pbuf), dest_addr >> 24,
             (dest_addr >> 16) & 0xff, (dest_addr >> 8) & 0xff,
             dest_addr & 0xff, st->output_dest_port);
  if (st->vlan_count != 0) {
    int i;
    ...
    for (i = 0; i < st->vlan_count; ++i) {
      ...
      fprint_msg("  VLAN %d: cfi:[%s], pcp[%s]\n", vi->vid,
                 map_to_string(vi->cfimap, sizeof(pbuf1), pbuf1),
                 map_to_string(vi->pcpmap, sizeof(pbuf2), pbuf2));
    }
  }
  ...
  if (st->seen_good == 0) {
    ...
    fprint_msg("  Pkts: Good=%d, Dodgy=%d, Bad=%d, Overlength=%u\n",
               st->seen_good - st->seen_dodgy, st->seen_dodgy, st->seen_bad,
               st->pkts_overlength);
    ...
    fprint_msg("  PCR PID: %d (%#x)%s\n", st->pcr_pid, st->pcr_pid,
               !st->multiple_pcr_pids
                   ? ""
                   : " ### Other PCR PIDs in stream - not tracked");
    ...
    for (tsect = st->section_first; tsect != nullptr; tsect = tsect->next) {
      ...
      int64_t time_len2 =
          tsect->time_final - tsect->time_start; // Stream duration
      ...
      fprint_msg("    TS Bytes: %llu (%llu bits/sec)\n",
                 ...
                 time_len2 == 0LL
                     ? 0LL
                     : (tsect->ts_byte_final - tsect->ts_byte_start) * 8ULL *
                           90000ULL / time_len2);
      ...
      if (tsect->pcr_count == 0) {
        ...
        fprint_msg("    Drift: diff=%s; rate=%s/min; 1s per %llds%s\n",
                   ...
                   fmtx_timestamp(
                       time_len == 0 ? 0LL : drift * 60LL * 90000LL / time_len,
                       ctx->tfmt),
      ...
      if (st->rtp_info.n != 0) {
        fprint_msg("    PCR/RTP skew: min=%s max=%s (diff=%s)\n",
                   ...
                   fmtx_timestamp(tsect->rtp_skew_max - tsect->rtp_skew_min,
                                  ctx->tfmt));
      }
    }
  }
...
unsigned int stream_hash(uint32_t const dest_addr, const uint32_t dest_port) {
  uint32_t x = dest_addr ^ dest_port;
  x ^= x >> 16;
  return (x ^ (x >> 8)) & 0xff;
}

static int stream_vlan_match(const pcapreport_stream_t *const st,
                             const ethernet_packet_t *const epkt) {
  int i;
...
pcapreport_stream_t *stream_find(pcapreport_ctx_t *const ctx,
  ...
  while ((st = *pst) != nullptr) {
    if (st->output_dest_addr == dest_addr &&
        st->output_dest_port == dest_port && stream_vlan_match(st, epkt)) {
      return st;
    }
    pst = &st->hash_next;
  }

  if ((st = stream_create(ctx, pcap_pkt_hdr, epkt, dest_addr, dest_port)) ==
      nullptr)
    return nullptr;
...
static int stream_sort_fn(const void *va, const void *vb) {
  const pcapreport_stream_t *const *const pa =
      (const pcapreport_stream_t *const *)va;
  const pcapreport_stream_t *const *const pb =
      (const pcapreport_stream_t *const *)vb;
  return (*pa)->stream_no - (*pb)->stream_no;
}

static int ip_reassemble(pcapreport_reassembly_t *const reas,
                         const ipv4_header_t *const ip, byte *const in_data,
  ...
  *out_plen = 0;
  ...
  // Very limited reassembly
  {
    ...
    if (frag->in_use && frag->ident != ip->ident) {
      fprint_err("### Multi-packet fragment reassembly NIF - previous packet "
                 "discarded\n");
      frag->in_use = 0;
    }
...
static int ip_reassembly_init(pcapreport_reassembly_t *const reas) {
  memset(reas, 0, sizeof(*reas));
  return 0;
}

static void print_usage() {
  print_msg("Usage: pcapreport [switches] <infile>\n"
            "\n");
  ...
  print_msg(
      ...
      "\n"
      "  -h                 This help\n"
      "  -h detail          More detail on what some terms used by pcapreport "
      "mean\n"
      "  --name <file>\n"
      "  -n <file>          Set the default base name for output files; by "
      "default\n"
      "                     this will be the input name without any .pcap "
      "suffix\n"
      "  -x, --extract      Extract TS(s) to files of the default name\n"
      "  -c, --csvgen       Create a .csv file for each stream containing "
      "timing info\n"
      "  -output <file>\n"
      ...
      "  -a                 Analyse.  Produces summary info on every TS in the "
      "pcap\n"
      "  -d <dest ip>:<port>\n"
      "  -d <dest ip>       Select data with the given destination IP and "
      "port.\n"
      "                     If the <port> is not specified, it defaults to 0\n"
      "                     (see below).\n"
      "  -g, --good-ts-only Only extract/analyse packets that seem entirely "
      "good.\n"
      "                     By default there is a bit of slack in determining "
      "if a\n"
      "                     packet is good and some dodgy packets are let "
      "through\n"
      "                     This switch ensures that all packets pass simple "
      "testing\n"
      "  -keep-bad          Extract all packets including bad ones.  Is "
      "implied if\n"
      ...
      "  -extra-dump, -e    Dump only data which isn't being sent to the -o "
      "file.\n"
      ...
      "  -skew <number>     Gives the skew discontinuity threshold in 90kHz "
      "units.\n"
      "                     A value of 0 disables this. [default = 6*90000]\n"
      "  -split-section     Split extracted streams into multiple files on "
      "section\n"
      ...
      "traditional)\n"
      "\n"
      "Specifying 0.0.0.0 for destination IP will capture all hosts, "
      "specifying 0\n"
...
static char manpage[] =
    ...
    "----------------------\n"
    "\n"
    "The times associated with packets and PCR are held internally in 90kHz "
    "units\n"
    ...
    "------\n"
    "\n"
    "A set of packets to the same IP & Port.  TS streams are detected by "
    "looking\n"
    ...
    "generate a separate .ts file if the extraction option is in effect, nor "
    "will\n"
    "it generate a new .csv file.)\n"
    "\n"
    "As it stands pcapreport will only report on a single PCR pid within a TS. "
    "If\n"
    "multiple pids with PCRs are detected then this will be reported but the "
    "other\n"
    ...
    "any\n"
    "PCR found in the TS contained within that packet.  The accuracy of this "
    "figure\n"
    "obviously depends on how good the clock was in the capture process.  Skew "
    "is\n"
    "arbitrarily set to zero at the start of a section.  A skew of >6s is "
    "assumed\n"
    ...
    "represents the difference in speed between the transmitters clock and "
    "the\n"
    "receivers clock.  The algorithm for determining this isn't very "
    "sophisticated\n"
    "so if you have a large maximum jitter or a short sample this should be "
    "taken\n"
    "with a pinch of salt.  Beware also that PC clocks (like the one in the "
    "m/c\n"
    "doing the tcpdump) are not always amongst the most stable or accurate; "
    "however\n"
    ...
    "------\n"
    "\n"
    "This is measured as the difference between the maximum and minimum skews "
    "over\n"
    "a 10sec (max 1024 samples) period.  This should be long enough to capture "
    "a\n"
...
int main(int argc, char **argv) {
  ...
  while (ii < argc) {
    if (argv[ii][0] == '-') {
      ...
      const char *const arg = c >= 'a' && c <= 'z' && argv[ii][2] == 0
                                  ? onechararg[c - 'a']
                              : argv[ii][1] == '-' ? argv[ii] + 2
                                                   : argv[ii] + 1;
      ...
      if (strcmp("help", arg) == 0) {
        ...
        CHECKARG("pcapreport", ii);
        err =
            host_value("pcapreport", argv[ii], argv[ii + 1], &hostname, &port);
        if (err)
          return 1;
        ++ii;
        ...
        if (ipv4_string_to_addr(&ctx->filter_dest_addr, hostname)) {
          fprint_err("### pcapreport: '%s' is not a host IP address (names are "
                     "not allowed!)\n",
                     hostname);
          return 1;
        }
  ...
  if (ctx->base_name == nullptr) {
    ...
    const char *const input_name =
        ctx->input_name == nullptr ? "pcap" : ctx->input_name;
  ...
  fprint_msg("Capture made by version %u.%u local_tz_correction "
             ...
             ctx->pcap_hdr.thiszone, ctx->pcap_hdr.sigfigs,
             ctx->pcap_hdr.snaplen, ctx->pcap_hdr.network);
      ...
      case 1: // Got a packet.
      {
        ...
        if (ctx->verbose) {
          fprint_msg("pkt: Time = %d.%d orig_len = %d \n", rec_hdr.ts_sec,
                     rec_hdr.ts_usec, rec_hdr.orig_len);
        }
...

        {
          ...
          rv = ethernet_packet_from_pcap(&rec_hdr, data, len, &epkt, &out_st,
                                         &out_len);
          ...
          if (ctx->verbose) {
            fprint_msg("++ 802.11: src %02x:%02x:%02x:%02x:%02x:%02x "
                       ...
                       "typeorlen 0x%04x\n",
                       epkt.src_addr[0], epkt.src_addr[1], epkt.src_addr[2],
                       epkt.src_addr[3], epkt.src_addr[4], epkt.src_addr[5],
                       epkt.dst_addr[0], epkt.dst_addr[1], epkt.dst_addr[2],
                       epkt.dst_addr[3], epkt.dst_addr[4], epkt.dst_addr[5],
                       epkt.typeorlen);
          }
          ...
          rv = ipv4_from_payload(data, len, &ipv4_hdr, &out_st, &out_len);
          if (rv) {
            goto dump_out;
          }
          ...
          if (ctx->verbose) {
            ...
            fprint_msg("++ IPv4: version = 0x%x hdr_length = 0x%x"
                       ...
                       ipv4_hdr.version, ipv4_hdr.hdr_length,
                       ipv4_hdr.serv_type, ipv4_hdr.length, ipv4_hdr.ident,
                       ipv4_hdr.flags, ipv4_hdr.frag_offset, ipv4_hdr.ttl,
                       ipv4_hdr.proto, ipv4_hdr.csum);
          }

          data = &data[out_st];
          len = out_len;

          if (ip_reassemble(&ctx->reassembly_env, &ipv4_hdr, data, &data,
                            &len) != 0) {
            goto dump_out;
          }
          ...
          rv = ipv4_udp_from_payload(data, len, &udp_hdr, &out_st, &out_len);
          if (rv) {
            goto dump_out;
          }
          ...
          if (ctx->verbose) {
            fprint_msg("++ udp: src port = %d "
                       "dest port = %d len = %d \n",
                       udp_hdr.source_port, udp_hdr.dest_port, udp_hdr.length);
          }

          data = &data[out_st];
          len = out_len;

          if ((ctx->filter_dest_addr == 0 ||
               (ipv4_hdr.dest_addr == ctx->filter_dest_addr)) &&
              (ctx->filter_dest_port == 0 ||
               (udp_hdr.dest_port == ctx->filter_dest_port))) {
            pcapreport_stream_t *const st = stream_find(
                ctx, &rec_hdr, &epkt, ipv4_hdr.dest_addr, udp_hdr.dest_port);
            ...
            if (stream_ts_check(ctx, st, data, len)) {
              ...
              if (ctx->time_report || ctx->analyse || ctx->csv_gen ||
                  (ctx->extract && ctx->file_split_section)) {
                rv = digest_times(ctx, st, &rec_hdr, &epkt, &ipv4_hdr, &udp_hdr,
                                  &rtp_hdr, data, len);
                if (rv) {
                  return rv;
                }
  ...
  if (ctx->analyse) {
    ...
    fprint_msg("Pcap start time: %llu (%d-%02d-%02d %d:%02d:%02d.%06d)\n",
               ctx->time_start, t->tm_year + 1900, t->tm_mon + 1, t->tm_mday,
    ...
    if (ctx->stream_count != 0) {
      unsigned int i;
      unsigned int j = 0;
      pcapreport_stream_t **const streams = (pcapreport_stream_t **)malloc(
          sizeof(pcapreport_stream_t *) * ctx->stream_count);
      ...
      qsort(streams, ctx->stream_count, sizeof(pcapreport_stream_t *),
            stream_sort_fn);
