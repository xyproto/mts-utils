...
static unsigned int jitter_add(
    jitter_env_t* const je, const int delta, const uint32_t time, const uint32_t range)
{
...
{
    return (((int64_t)pcap_pkt_hdr->ts_usec * 9) / 100) + ((int64_t)pcap_pkt_hdr->ts_sec * 90000);
}

static char* vlan_name(
    const char* prefix, const pcapreport_stream_t* const st, const size_t blen, char* const buf)
{
    if (st->vlan_count == 0) {
        buf[0] = '\0';
    } else {
...
static char* section_name(const pcapreport_ctx_t* const ctx, const pcapreport_stream_t* const st,
    char* const pbuf, const size_t pbuf_len)
{
    ...
    snprintf(pbuf, pbuf_len, "_S%d", st->section_last == nullptr ? 0 : st->section_last->section_no);
    return pbuf;
}

static void stream_gen_names2(const pcapreport_ctx_t* const ctx, pcapreport_stream_t* const st,
    const rtp_header_t* const rtp_header)
{
    ...
    if (ctx->filter_dest_addr == 0 || ctx->filter_dest_port == 0) {
        snprintf(identifier, 90, "%s_%u.%u.%u.%u_%u%s", vlan_name("_V", st, sizeof(pbuf), pbuf),
            dest_addr >> 24, (dest_addr >> 16) & 0xff, (dest_addr >> 8) & 0xff, dest_addr & 0xff,
            dest_port, section_name(ctx, st, pbuf2, sizeof(pbuf2)));
    } else {
        identifier[0] = '\0';
...
static void stream_gen_names(const pcapreport_ctx_t* const ctx, pcapreport_stream_t* const st,
    const rtp_header_t* const rtp_header)
{
...
static int digest_times(pcapreport_ctx_t* const ctx, pcapreport_stream_t* const st,
    const pcaprec_hdr_t* const pcap_pkt_hdr, const ethernet_packet_t* const epkt,
    const ipv4_header_t* const ipv4_header, const ipv4_udp_header_t* const udp_header,
    const rtp_header_t* const rtp_header, const byte* const data, const uint32_t len)
{
    int rv;
    ...
    if (rtp_header->is_rtp_ts) {
        ...
        rtp_seq_delta
            = ri->n == 0 ? 0 : (rtp_header->sequence_number - (ri->last_seq + 1)) & 0xffffU;
    ...
    while (1) {
        ...
        // Right. Split it ..
        {
            ...
            rv = split_TS_packet(pkt, &pid, &pusi, &adapt, &adapt_len, &payload, &payload_len);
            if (rv) {
                fprint_msg(">%d> WARNING: TS packet %d [ packet %d @ %d.%d s ] cannot be split.\n",
                    st->stream_no, st->ts_counter, ctx->pkt_counter, pcap_pkt_hdr->ts_sec,
                    ...
                    if (has_pcr) {
                        ...
                        if (pid != st->pcr_pid) {
                            ...
                            if (tsect->pcr_count == 0) {
                                if (tsect->section_no != 0) {
                                    fprint_msg(">%d> Skew discontinuity! Skew = %lld (> %lld) at"
                                        ...
                                        st->stream_no, skew, st->skew_discontinuity_threshold,
                                        st->ts_counter, ctx->pkt_counter, pcr,
                                        pcap_pkt_hdr->ts_sec, pcap_pkt_hdr->ts_usec);
                                }
                            ...
                            if (rtp_header->is_rtp_ts) {
                                ...
                                int32_t rtp_skew = (int32_t)(
                                    rtp_header->timestamp - (uint32_t)(t_pcr & 0xffffffffU));
                            ...
                            if (ctx->time_report) {
                                ...
                                double skew_rate = (rel_tim == 0)
                                    ? 0.0
                                    : (double)skew / ((double)((double)rel_tim / (60 * 90000)));

                                fprint_msg(
                                    ">%d> [ts %d net %d ] PCR %lld Time %d.%d [rel %d.%d]  - skew "
...
static int write_out_packet(pcapreport_ctx_t* const ctx, pcapreport_stream_t* const st,
    const byte* data, const uint32_t len)
{
    int rv;
...
static int stream_ts_check(const pcapreport_ctx_t* const ctx, pcapreport_stream_t* const st,
...
static int write_rtp_raw_packet(pcapreport_ctx_t* const ctx, pcapreport_stream_t* const st,
    const byte* data, const uint32_t len)
{
    if (st->output_name) {
        int rv;
...
static int stream_rtp_check(const pcapreport_ctx_t* const ctx, pcapreport_stream_t* const st,
    const byte* const data, const uint32_t len, rtp_header_t* const rh)
{
    uint32_t offset;
    uint32_t padlen = 0;
...
static void stream_merge_vlan_info(
    pcapreport_stream_t* const st, const ethernet_packet_t* const epkt)
{
    int i;
...
static pcapreport_stream_t* stream_create(pcapreport_ctx_t* const ctx,
    const pcaprec_hdr_t* const pcap_pkt_hdr, const ethernet_packet_t* const epkt,
...
static void stream_analysis(const pcapreport_ctx_t* const ctx, const pcapreport_stream_t* const st)
{
    ...
    fprint_msg("Stream %d: Dest:%s %u.%u.%u.%u:%u\n", st->stream_no,
        vlan_name(" VLAN:", st, sizeof(pbuf), pbuf), dest_addr >> 24, (dest_addr >> 16) & 0xff,
        (dest_addr >> 8) & 0xff, dest_addr & 0xff, st->output_dest_port);
    if (st->vlan_count != 0) {
        int i;
    ...
    if (st->seen_good == 0) {
        ...
        for (tsect = st->section_first; tsect != nullptr; tsect = tsect->next) {
            ...
            fprint_msg("    TS Bytes: %llu (%llu bits/sec)\n",
                ...
                time_len2 == 0LL
                    ? 0LL
                    : (tsect->ts_byte_final - tsect->ts_byte_start) * 8ULL * 90000ULL / time_len2);
            ...
            if (tsect->pcr_count == 0) {
                ...
                fprint_msg("    Drift: diff=%s; rate=%s/min; 1s per %llds%s\n",
                    ...
                    fmtx_timestamp(
                        time_len == 0 ? 0LL : drift * 60LL * 90000LL / time_len, ctx->tfmt),
...
static int stream_vlan_match(
    const pcapreport_stream_t* const st, const ethernet_packet_t* const epkt)
{
    int i;
...
static int ip_reassemble(pcapreport_reassembly_t* const reas, const ipv4_header_t* const ip,
    byte* const in_data, byte** const out_pdata, uint32_t* const out_plen)
{
...
int main(int argc, char** argv)
{
        ...
        if (argv[ii][0] == '-') {
            ...
            if (strcmp("help", arg) == 0) {
                ...
                if (ipv4_string_to_addr(&ctx->filter_dest_addr, hostname)) {
                    fprint_err(
                        "### pcapreport: '%s' is not a host IP address (names are not allowed!)\n",
                        hostname);
                    return 1;
                }
    ...
    fprint_msg("Capture made by version %u.%u local_tz_correction "
        ...
        ctx->pcap_hdr.version_major, ctx->pcap_hdr.version_minor, ctx->pcap_hdr.thiszone,
        ctx->pcap_hdr.sigfigs, ctx->pcap_hdr.snaplen, ctx->pcap_hdr.network);
        ...
        while (!done) {
            ...
            case 1: // Got a packet.
            {
...

                {
                    ...
                    rv = ethernet_packet_from_pcap(&rec_hdr, data, len, &epkt, &out_st, &out_len);
                    ...
                    if (ctx->verbose) {
                        fprint_msg("++ 802.11: src %02x:%02x:%02x:%02x:%02x:%02x "
                                   ...
                                   "typeorlen 0x%04x\n",
                            epkt.src_addr[0], epkt.src_addr[1], epkt.src_addr[2], epkt.src_addr[3],
                            epkt.src_addr[4], epkt.src_addr[5], epkt.dst_addr[0], epkt.dst_addr[1],
                            epkt.dst_addr[2], epkt.dst_addr[3], epkt.dst_addr[4], epkt.dst_addr[5],
                            epkt.typeorlen);
                    }
                    ...
                    if (ctx->verbose) {
                        fprint_msg("++ IPv4: src = %s", ipv4_addr_to_string(ipv4_hdr.src_addr));
                        ...
                        fprint_msg("++ IPv4: version = 0x%x hdr_length = 0x%x"
                            ...
                            ipv4_hdr.version, ipv4_hdr.hdr_length, ipv4_hdr.serv_type,
                            ipv4_hdr.length, ipv4_hdr.ident, ipv4_hdr.flags, ipv4_hdr.frag_offset,
                            ipv4_hdr.ttl, ipv4_hdr.proto, ipv4_hdr.csum);
                    }
            ...
            default:
                // Some other error.
                fprint_err(
                    "### pcapreport: Can't read packet %d - code %d\n", ctx->pkt_counter, err);
                ++done;
                break;
            }
        }
